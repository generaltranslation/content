...
The `derive()` function tells the GT compiler to create **multiple translation entries** from a single `gt()` call based on different runtime values. Instead of writing separate `gt()` calls for each variant, you use `derive()` to express all the variants in one place, and GT generates the appropriate translations at build time.
...
At build time, the compiler sees the `derive()` call and generates two separate translation entries:
