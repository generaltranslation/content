...
```jsx
gt("I am tired", {
  $context: derive(isMasculine ? "masculine" : "feminine")
})
```

When the CLI processes this, it registers two source entries — one with `$context: "masculine"` and one with `$context: "feminine"` — each of which gets its own translation.
