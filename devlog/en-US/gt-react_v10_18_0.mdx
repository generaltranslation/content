...
Like content strings, `$context` values must be **static** — the CLI needs to resolve them at build time, not at runtime. You can't pass a runtime expression directly as `$context` because the CLI wouldn't know what values it could take. `derive()` solves this by tracing through the call stack to resolve all possible static strings that could be passed, letting the CLI register a source entry for each one.
