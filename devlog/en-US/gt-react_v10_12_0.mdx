...
The `loadTranslations` function is async — it loads the translations first, and then the React app is imported and executed. This is how `t()` has access at the module level: by the time any module calls `t()`, translations are already available.
