...
The companion file must live in the same directory as the source file and follow the naming convention `{name}.metadata.json` (or `.metadata.yaml` for YAML files). The CLI picks it up automatically when you run [`npx gt translate`](/docs/cli/reference/commands/translate).
