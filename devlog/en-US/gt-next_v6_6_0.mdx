...
Our library has been incredibly focused on internationalizing "green field" apps up until this point. Consider the [`<T>`](/docs/react/reference/components/t) component or the [`useGT()`](/docs/react/reference/hooks/use-gt) hook as examples.

The only infrastructure we've built specifically for "brown field" apps is the `const t = useTranslations()` and `const t = getTranslations()` hooks, modeled after next-intl's dictionary pattern. However, our [`t()`](/docs/react/reference/functions/t-function) function has been limited, only returning strings, not dictionary subtrees.
