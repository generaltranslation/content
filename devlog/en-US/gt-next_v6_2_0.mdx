...
Functions like [`getGT()`](/docs/react/nextjs/reference/functions/get-gt) and server-side [`<T>`](/docs/react/reference/components/t) no longer need to compute and cache hashes on every call, resulting in more consistent performance.
...
[`getGT()`](/docs/react/nextjs/reference/functions/get-gt) and [`useGT()`](/docs/react/reference/hooks/use-gt) follow the familiar i18n pattern:
...
Now, thanks to the compiler, translatable content is scanned ahead of time and passed directly into [`useGT()`](/docs/react/reference/hooks/use-gt) or [`getGT()`](/docs/react/nextjs/reference/functions/get-gt) at compile time.
...
Previously, detecting incorrect usage of [`<T>`](/docs/react/reference/components/t) components required running `gtx-cli` with a custom linter.
...
If any translatable content isn't wrapped in a [`<T>`](/docs/react/reference/components/t) component, you'll get a **build-time error**, ensuring issues are caught early.
