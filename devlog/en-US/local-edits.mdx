...
_Version note: This post describes the 2.4.0 behavior. In `gt` and `gtx-cli` 2.16.0, pass `--save-local` to [`translate`](/docs/cli/reference/commands/translate) to run the save step; see the current `gt translate` reference._
...
The new [`save-local`](/docs/cli/reference/commands/save-local) command lets you commit local translation edits back to your General Translation project. You stay in your workspace, and your changes stay in sync.

```bash
npx gtx-cli save-local
```

Running this command uploads any modified translation files, merges your local changes into the project, and ensures that the next [`translate`](/docs/cli/reference/commands/translate) run reflects your edits.

---

## Translate (Now With Local Awareness)

Until now, [`translate`](/docs/cli/reference/commands/translate) would always assume the remote version of a translation was the source of truth.

If you made local changes and ran [`translate`](/docs/cli/reference/commands/translate) again, those edits would be overwritten. That's no longer the case.
...
the CLI automatically checks for local edits and saves them in the background — the same way [`save-local`](/docs/cli/reference/commands/save-local) does — before re-downloading translations. This guarantees that your local versions are preserved and that re-translations always persist your edits.
...
- [`save-local`](/docs/cli/reference/commands/save-local) lets you push local translation edits from your CLI.
- [`translate`](/docs/cli/reference/commands/translate) now automatically preserves and merges local edits.
