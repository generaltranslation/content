...
When you run the CLI ([`npx gt translate`](/docs/cli/reference/commands/translate)), it automatically detects `dictionary.json` in the project root and translates it for your configured locales.

<Accordions>
  <Accordion title="Bringing your own dictionaries">
    If you want to load your own pre-translated dictionaries, you can also pass an async `loadDictionary()` function. You still need to provide the default dictionary explicitly — `loadDictionary` is only for translated locales:
