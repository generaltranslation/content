...
`gt-i18n` now supports dictionary-backed translations via [`getTranslations()`](/docs/node/reference/functions/get-translations). This brings `gt-i18n` in line with `gt-next`, `gt-react`, and other i18n libraries that use dictionaries as their primary translation pattern.
...
<Accordions>
  <Accordion title="Bringing your own dictionaries">
    If you want to load your own pre-translated dictionaries, you can also pass a [`loadDictionary()`](/docs/react/reference/functions/load-dictionary) async function. You still need to provide the default dictionary explicitly — [`loadDictionary`](/docs/react/reference/functions/load-dictionary) is only for translated locales:
...
- [`getTranslations` API reference](/docs/react/reference/functions/get-translations-snapshot)
