...
Under the hood, `gt-node` uses an `AsyncStorageAdapter` backed by `AsyncLocalStorage` to scope locale per request. The core translation logic — string resolution, message decoding, variable interpolation — lives in `gt-i18n` and is shared across all adapters.
