...
In [gt-next@6.8.0](/devlog/gt-next_v6_8_0), we introduced the `<Static>` component to address sentence fragmentation and code reuse in JSX content.
...
However, this left a gap for string-based translations using `gt()` and [`msg()`](/docs/react/reference/functions/msg).
...
**gt-next 6.12.0** bridges this gap by introducing `declareStatic()` - the string equivalent of the `<Static>` component - along with supporting functions [`declareVar()`](/docs/react/reference/functions/declare-var) and [`decodeVars()`](/docs/react/reference/functions/decode-vars).
...
The string equivalent of [`<Var>`](/docs/react/reference/components/var) - marks dynamic content within `declareStatic()` functions that should be excluded from hash calculations and handled as variables at runtime.
...
Because [`declareVar()`](/docs/react/reference/functions/declare-var) adds ICU-compatible markers in source text, adding [`declareVar()`](/docs/react/reference/functions/declare-var) on its own can create issues for existing string-processing logic.
To extract the original value, you just need to wrap the source string in [`decodeVars()`](/docs/react/reference/functions/decode-vars).
...
Both `gt()` and [`msg()`](/docs/react/reference/functions/msg) now support string concatenation for **static strings** only:
