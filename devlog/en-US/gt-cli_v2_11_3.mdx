...
[`derive()`](/docs/react/reference/functions/derive) now supports object and array access. Previously, [`derive()`](/docs/react/reference/functions/derive) could only resolve function calls — now it can resolve values from objects, arrays, and dictionaries, making it easier to work with lookup patterns.
...
Use [`derive()`](/docs/react/reference/functions/derive) with object property lookups. The CLI statically resolves all possible values at extraction time:
...
The same patterns work in Python with [`t()`](/docs/python/reference/functions/t). Use [`derive()`](/docs/python/reference/functions/derive) as the first argument or inside an f-string:
