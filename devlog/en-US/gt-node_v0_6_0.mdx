---
title: gt-node@0.6.0
date: 2026-04-14
authors: [ernest]
tags: ['gt-node', 'runtime-translation', 'tx']
---

## Overview

`gt-node` now has [`tx()`](/docs/node/reference/functions/tx) — a function for translating strings at runtime. Pass any string to [`tx()`](/docs/node/reference/functions/tx) and get back a translated version, no build step needed.

This unlocks server-side translation of dynamic content: user-generated text, database values, API responses, or anything that isn't known ahead of time.

## Usage

### Basic

```js
import { tx } from 'gt-node';

const translated = await tx('Processing complete');
```

### With variables

```js
const translated = await tx('Hello, {name}!', { name: 'María' });
```

### Specifying a locale

```js
const status = await tx('Order shipped', { $locale: 'ja' });
```

## Links

- [PR #1207](https://github.com/generaltranslation/gt/pull/1207)
