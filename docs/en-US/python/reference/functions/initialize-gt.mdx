...
- **Locale detection.** On Flask it registers a `before_request` hook; on FastAPI it adds an HTTP middleware and wraps the app lifespan. Each request sets the locale from `get_locale` if provided, otherwise from the `Accept-Language` header.
...
| [`custom_mapping`](#custom-mapping) | Custom locale codes and property overrides. | `CustomMapping` | Yes | Config |
