...
<Callout type="warn">
  **Frontend changes required.** The `gt-sanity` plugin creates separate translated documents in your Sanity dataset, but it does not automatically update your schemas or GROQ queries.
  You will need to update your frontend queries to fetch the correct language version of each document.
  See [Sanity's localization docs](https://www.sanity.io/docs/studio/localization) for GROQ query patterns.
</Callout>
