...
<Callout type="info">
  **No schema changes required.** The `gt-sanity` plugin works with your existing Sanity schemas out of the box — no i18n fields or data model restructuring needed.
  Translations are stored as separate documents, so you'll need to update your frontend queries to fetch the right language version. See [Querying translated content](#querying-translated-content) below.
</Callout>
...
<Tabs items={["Without GT (source only)", "With GT (localized)"]}>
  <Tab value="Without GT (source only)">
