...
<Callout type="info">
  **No schema changes or GROQ modifications needed.** The `gt-sanity` plugin works with your existing Sanity setup out of the box.
  Translations are stored as separate documents, so you don't need to restructure your data model, add i18n fields, or rewrite any queries.
</Callout>
