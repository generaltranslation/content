...
Fields you don't want translated or sent to the GT API, but that should still be copied from the source document and made unique on each translated document. Dedupe fields are stripped before serialization, then restored from the source document with the target locale appended.
