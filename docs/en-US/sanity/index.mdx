...
<Callout type="info">
  **No schema changes required.** The plugin works with your existing Sanity schemas as-is — no i18n fields or data model changes needed.
  Translations are stored as separate documents, so you will need to update your frontend GROQ queries to fetch the correct language version.
  See the [quickstart](/docs/sanity/guides/quickstart) to get set up.
</Callout>
