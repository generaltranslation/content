...
Fields like slugs can be configured with `dedupeFields` so translated documents keep a source-derived value while still receiving a unique locale suffix, such as `about-es` or `about-fr`.
