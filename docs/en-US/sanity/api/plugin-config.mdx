...
<TypeTable
  type={{
    ...
    dedupeFields: {
      type: 'DedupeFields[]',
      description: 'Fields to exclude from translation, then restore from the source document with the target locale appended so the translated value is unique.',
      optional: true,
    },
    skipFields: {
      type: 'SkipFields[]',
...
Fields that are not translated or sent to the API, but are copied from the source document with the target locale appended. Use this for fields that should remain source-derived but unique per translated document, such as Sanity slugs.
...
For Sanity slug fields, the plugin updates the slug object's `current` value. For example, `{ _type: 'slug', current: 'about' }` becomes `{ _type: 'slug', current: 'about-es' }` on the Spanish document.
