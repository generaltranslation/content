...
Fields that are removed entirely from translated documents. Unlike `ignoreFields`, these fields will not appear on translations at all. Use this for fields like unique document slugs, SEO canonical URLs, or source-only metadata that should either be set independently per language or only exist on the source.
...
gtPlugin({
  ...
  locales: ['es'],
  skipFields: [
    // Slugs should be unique per language, don't copy from source
    {
