...
|---|---|
| JSX content (headings, paragraphs, buttons) | [`<T>`](/docs/next/api/components/t) |
| HTML attributes (`placeholder`, `aria-label`, `alt`) | [`useGT`](/docs/next/api/strings/use-gt) |
| Strings in objects, arrays, or config files | [`msg`](/docs/next/api/strings/msg) + [`useMessages`](/docs/next/api/strings/use-messages) |
| Server component strings | [`getGT`](/docs/next/api/strings/get-gt) |
| Migrating from another i18n library | [`useTranslations`](/docs/next/api/dictionary/use-translations) |

<Callout type="info">
When in doubt, use [`<T>`](/docs/next/api/components/t). It handles the widest range of content and gives translators the most context, which produces better translations.
</Callout>
