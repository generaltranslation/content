...
The trade off is that there is a delay while waiting for an on-demand translation to load, which is significantly slower than build-time translation.
...
## Notes
 * `tx` is exclusively for server-side usage and cannot be used in client-side components.
