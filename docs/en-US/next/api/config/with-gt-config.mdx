...
| `pathRegex`              | Regular expression string that pathnames must match for i18n middleware routing to run.                                                                                                                       |
...
## Filter middleware paths

Pass a regular expression string to `pathRegex` to apply `gt-next` i18n middleware only to matching pathnames. This filter runs in addition to the Next.js `matcher` in `proxy.ts`. For requests that do not match `pathRegex`, the middleware skips locale detection, middleware cookies, rewrites, and redirects. Client components also skip locale-path consistency checks for excluded paths. When `pathRegex` is omitted, the middleware behavior is unchanged.

For example, this configuration leaves `/uk` and its subpaths unchanged while applying i18n middleware to every other matched path:
