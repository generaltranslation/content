...
<Callout type='info'>
  **Quick Setup:** Try `npx gt@latest` for automatic configuration. Run the
  [Setup Wizard](/docs/cli/init) or use [Locadex](/docs/locadex/next) to set it up for you.
</Callout>

## Installation

Install the `gt-next` and `gt` packages:
...
The `files.gt.output` field tells the CLI where to save translation files when you run `npx gt translate`. The `[locale]` placeholder is replaced with each target locale code (e.g., `.gt/fr.json`).
...
For server components, use [`getGT`](/docs/next/api/strings/get-gt) instead of [`useGT`](/docs/next/api/strings/use-gt). Note that `getGT` is imported from `'gt-next/server'`:
