...
<Callout type='info'>
  **Quick Setup:** Try `npx gtx-cli@latest` for automatic configuration. Run the
  [Setup Wizard](/docs/cli/init) or use [Locadex](/docs/locadex/next) to set it up for you.
</Callout>
...
The [`withGTConfig`](/docs/next/api/config/with-gt-config) function initializes the SDK in your Next.js application. Add it to your `next.config.[js|ts]` file:
...
The [`GTProvider`](/docs/next/api/components/gtprovider) component provides translation context to client-side components. It manages locale state, translations, and enables the [`useGT`](/docs/next/api/strings/use-gt) and [`useTranslations`](/docs/next/api/dictionary/use-translations) hooks.
...
```json title="gt.config.json"
{
  ...
  "files": {
    "gt": {
      "output": ".gt/[locale].json"
    }
  }
}
```

The `files.gt.output` field tells the CLI where to save translation files when you run `npx gtx-cli translate`. The `[locale]` placeholder is replaced with each target locale code (e.g., `.gt/fr.json`).
...
<Callout type="warn">
  **You must use a `gtx-dev-` key for local development.** Dev keys (starting with `gtx-dev-`) enable on-demand translation hot-reloading in development mode. Production keys (`gtx-api-`) will **not** work in dev — your app will stay in the default language. Save `gtx-api-` keys for CI/CD when running `gtx-cli translate` during deploy.
...
For attributes, labels, and plain text using the [`useGT`](/docs/next/api/strings/use-gt) hook:
...
For server components, use [`getGT`](/docs/next/api/strings/get-gt) instead of [`useGT`](/docs/next/api/strings/use-gt).
...
1. **Add a locale selection dropdown** using [`<LocaleSelector>`](/docs/next/api/components/locale-selector):
...
<Accordions>
  ...
  <Accordion title="Why are some translations inaccurate?">
    ...
    The [`<T>`](/docs/next/api/components/t), [`useGT`](/docs/next/api/strings/use-gt), and [`getGT`](/docs/next/api/strings/get-gt) functions all support a `context` prop.
    For example:
    ```jsx
...
2. **Add to your CI/CD environment**:
   ...
   <Callout type='warn'>
     **Never** prefix production keys with `NEXT_PUBLIC_` - they should remain
     server-side only.
   </Callout>
