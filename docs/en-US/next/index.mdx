...
<Callout type="info">
  **Have text in arrays or config objects?** If you store translatable text in arrays (like navigation items or feature lists) and render them with `.map()`, use [`msg`](/docs/next/api/strings/msg) to mark those strings for translation. See the [Shared Strings guide](/docs/next/guides/shared-strings) for the full pattern.
</Callout>
