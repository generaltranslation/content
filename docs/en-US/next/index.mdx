...
<Callout type='info'>
  **Want automatic setup?** Run `npx gt@latest init` to configure everything with the [Translation Wizard](/docs/cli/init). This guide covers manual setup.
</Callout>
...
- **`files.gt.output`** — where the CLI saves translation files. `[locale]` is replaced with each language code (e.g., `.gt/es.json`).

Add `.gt/` to your **`.gitignore`** — these files are generated, not hand-written:
