...
<Callout type="warn">
  **Security:** Never expose `GT_API_KEY` to the client bundle. Do **not** use `NEXT_PUBLIC_GT_API_KEY` — this would make your production secret publicly accessible in the browser.
  `GT_API_KEY` must remain a server-side-only environment variable.
</Callout>
