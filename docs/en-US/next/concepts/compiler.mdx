...
When autoderive is enabled, the compiler automatically treats interpolated values in `t()`, `gt()`, and `msg()` calls as [`derive()`](/docs/next/api/strings/derive) calls — generating separate translations for each possible value to preserve word agreement and conjugation across languages.

Autoderive is **disabled by default**. See the [CLI configuration reference](/docs/cli/reference/config#autoderive) for details on how to enable it and how it works.
