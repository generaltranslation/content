...
Pass a regular expression string to `regexFilter` to apply `gt-next` i18n middleware only to matching pathnames. This filter runs in addition to the Next.js `matcher` in `proxy.ts`. Requests that do not match `regexFilter` continue without locale detection, cookies, rewrites, or redirects. When `regexFilter` is omitted, the middleware behavior is unchanged.
