...
Enable cache components and [root params](https://nextjs.org/docs/app/api-reference/config/next-config-js/rootParams) in your Next.js config, and define custom `getLocale` and `getRegion` request functions that use `next/root-params` so that gt-next can resolve the locale inside cached components.
...
- The `next/root-params` API (Next.js 16.2+) allows reading route parameters like `[locale]` from anywhere in the component tree, including inside `"use cache"` boundaries. The root param value automatically becomes part of the cache key.
- Requires `experimental.rootParams: true` in your Next.js config and the `[locale]` segment to be a root parameter (no `app/layout.tsx` above it).
