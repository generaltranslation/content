...
GT provides the `resolveCanonicalLocale()` method to convert aliases back to their BCP 47 codes. You can access it via `getGTClass` from `gt-next`, which returns a `GTRuntime` instance with translation, formatting, and locale utilities. (The full `GT` class, which adds the project and file-management API client, remains available from `generaltranslation`.)

```ts
import { getGTClass } from 'gt-next';
