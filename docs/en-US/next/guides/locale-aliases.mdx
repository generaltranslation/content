...
GT provides the `resolveCanonicalLocale()` function to convert aliases back to their BCP 47 codes. It is exported from `gt-next` and reads the custom mapping from your configuration:

```ts
import { resolveCanonicalLocale } from 'gt-next';
