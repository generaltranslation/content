...
- **Delivery mode.** Provide `projectId` to load translations from the General Translation CDN, or `loadTranslations` to bring your own source. For on-demand translation in development, also provide `devApiKey`.
