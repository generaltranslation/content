...
*Note: `getTranslations` must be called within a [`withGT`](/docs/node/reference/functions/with-gt) callback so it knows which locale to use, and requires a `dictionary` (or `loadDictionary`) configured on [`initializeGT`](/docs/node/reference/functions/initialize-gt).*
