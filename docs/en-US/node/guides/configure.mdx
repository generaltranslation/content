...
title: Configure gt-node
description: Initialize General Translation gt-node with locales, credentials, and translation delivery.

---

`gt-node` is configured entirely through the `initializeGT` call you make once at startup. This guide covers what to pass and how translations are delivered.
...
- **Local files:** provide a `loadTranslations` callback that returns translations for a locale. See [Store translations locally](/docs/node/guides/local-translations).
