...
export GT_DEV_API_KEY="gtx-api-your-runtime-key"
export GT_API_KEY="gtx-api-..."
```

Use a [project API key](/docs/platform/dashboard/reference/api-keys#create-project-keys) with **Runtime translation** enabled. `GT_DEV_API_KEY` enables development hot reload; `GT_API_KEY` supplies credentials for other runtime requests. Both accept `gtx-api-` project keys.
...
In development, a project ID and project API key with runtime translation permission enable on-demand translation and hot reload, so new strings translate as you work. In production, translations come from your pre-generated files or the CDN. See the [Configuration reference](/docs/node/reference/config) for every option.
