...
<Callout>
  `getGT()` and `msg()`/`getMessages()` produce identical translations — the difference is only in code organization. Use `tx()` only when content isn't known at build time.
</Callout>
