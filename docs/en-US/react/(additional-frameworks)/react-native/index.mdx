...
- **Module-level setup.** You call [`initializeGT`](/docs/node/reference/functions/initialize-gt) once at your app's entry point, and [`<GTProvider>`](/docs/react/reference/components/gt-provider) loads translations internally rather than taking `locale` and `translations` props. See [Setup](/docs/react/react-native/setup).
