...
*Note: In `gt-react`, call `useTranslations` under a [`<GTProvider>`](/docs/react/reference/components/gt-provider), with a dictionary supplied to the provider or initialization call. In `gt-next`, it also works in synchronous server components. If you only use `<T>`, you do not need it.*
...
- In an async App Router component, use [`getTranslations`](/docs/react/nextjs/server-functions#get-translations) instead.
