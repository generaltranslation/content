...
In a server-rendered app, the hook reads the locale provided by the server through [`<GTProvider>`](/docs/react/reference/components/gt-provider). A locale setter writes the request cookie and refreshes or reloads the app; the rendered locale changes when the new server snapshot arrives. A provider-free React SPA reads its cookie-backed client store instead.
