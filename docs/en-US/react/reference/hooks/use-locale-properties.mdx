...
*Note: `useLocaleProperties` is client-side only, must be used under a [`<GTProvider>`](/docs/react/reference/components/gt-provider), and is synchronous.*
...
- `useLocaleProperties` is client-side only and synchronous.
