...
- Next.js App Router stores it in a cookie and usually refreshes Server Components with `router.refresh()`. With locale routing enabled, switching to the default locale from a URL resolved to another locale reloads the page so middleware can rewrite the pathname. Pages Router can replace the default reload with Next.js navigation.
