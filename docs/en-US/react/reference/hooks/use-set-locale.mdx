...
Calling the returned function persists the chosen locale in a cookie and reloads the page so the new locale's translations are applied. In TanStack Start with `localeRouting` enabled, it reloads at the corresponding locale pathname.
