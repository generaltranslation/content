...
*Note: `useGT` must be called inside a component rendered under a [`<GTProvider>`](/docs/react/reference/components/gt-provider). Write `const gt = useGT()` — the hook returns the function directly, not an object.*
