...
The `<Plural>` component swaps its wording based on a count so the sentence agrees with the number in each language. It selects the right form for a number. English needs two ("one item" / "two items"), while other languages need up to six.
