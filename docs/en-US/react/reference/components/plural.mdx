...
- **Requires a number.** `<Plural>` throws an error if `n` is missing or not a valid number.

## Which forms to add [#forms]

You only need the plural forms your language uses. The possible forms are `zero`, `one`, `two`, `few`, `many`, `other`, plus the aliases `singular` and `plural`.
...
The number used to determine the plural form. Required — the component throws if it is missing or not a valid number.
...
A prop per plural category — `zero`, `one`, `two`, `few`, `many`, `other` (or `singular`/`plural`). The available categories depend on the locale.
