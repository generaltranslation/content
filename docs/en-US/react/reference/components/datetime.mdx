...
Pass a `Date` as children and `<DateTime>` formats it for the active locale. This example pins the locale and time zone so its output is deterministic.
...
- **Local formatting.** The date is formatted in the current runtime using `Intl.DateTimeFormat`. Its value is never sent to the General Translation API.
...
| --- | --- | --- | --- | --- |
| [`children`](#children) | The date to format. A nullish value renders nothing. | `Date \| null \| undefined` | No | — |
...
**Type** `Date | null | undefined` · **Required**

The date or time to format, as a `Date` object. `null` and `undefined` render nothing.
