...
The `<RelativeTime>` component formats the distance between two times as human-readable text for the active locale. It works either by auto-selecting the best unit from a `Date`, or from an explicit value and unit.
