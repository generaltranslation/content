...
- **Local formatting.** The relative time is computed and formatted in the current runtime; the value is never sent to the General Translation API.
...
| --- | --- | --- | --- | --- |
| [`children`](#children) | A `Date` to compute relative time from. | `Date \| null` | Yes | — |
| [`date`](#date) | A `Date` to compute from. Takes precedence over `children` when non-null. | `Date \| null` | Yes | — |
...
**Type** `Date | null` · **Optional**

A `Date` object. The component automatically selects the best unit (seconds through years) and formats the time relative to `baseDate`. A nullish value is treated as no input.

### `date` [#date]

**Type** `Date | null` · **Optional**

A `Date` to compute relative time from. When both `date` and `children` are provided, a non-null `date` takes precedence.
