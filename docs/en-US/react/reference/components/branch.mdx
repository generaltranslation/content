...
- **Fallback.** When no prop matches the value, `children` renders as a fallback.
- **Requires a value.** `<Branch>` throws an error if `branch` is not provided or is invalid.

## Props [#props]

| Prop | Description | Type | Optional | Default |
| --- | --- | --- | --- | --- |
| [`branch`](#branch) | The value that selects the content. | `string \| number \| boolean` | No | — |
...
The value used to select which branch renders. The component throws if it is missing or invalid.
...
- Combine `<Branch>` with [`<T>`](/docs/react/reference/components/t) and [variable components](/docs/react/guides/format-variables) for translated, dynamic content.
