...
If [`loadTranslations`](/docs/react/reference/functions/load-translations) rejects, `getTranslationsSnapshot` logs a warning and resolves to `{}`. It omits the locale instead of hydrating an empty cache entry, so a later lookup retries after translations become available.
