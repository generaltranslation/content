...
In development, if [`loadTranslations`](/docs/react/reference/functions/load-translations) rejects, `getTranslationsSnapshot` rejects with that error. In production, the translation cache logs the error and resolves with an empty entry for the locale: `{ [locale]: {} }`.
