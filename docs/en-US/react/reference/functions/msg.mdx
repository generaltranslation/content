...
*Note: `msg` encodes its input, so you cannot render it directly. To recover the original string, decode it with `decodeMsg`.*
...
- **Decoding.** `decodeMsg` extracts the original interpolated string from an encoded message. `decodeOptions` extracts the options.
