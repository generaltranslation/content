...
- **Message format.** The default `ICU` format interpolates variables before encoding. Set `$format: 'STRING'` to preserve braces and source text literally; interpolation variables are ignored in this mode.
...
Interpolation variables plus options such as `$context`, `$id`, `$format`, and `$maxChars`. Variables are inserted into the string for `ICU` messages. With `$format: 'STRING'`, the source is encoded literally and variables are ignored.

## Returns [#returns]

**Type** `string`

An encoded string. `ICU` messages include interpolated variables; `STRING` messages preserve the source text literally. For an array input, an array of encoded strings is returned. Resolve the result with [`useMessages`](/docs/react/reference/hooks/use-messages).
