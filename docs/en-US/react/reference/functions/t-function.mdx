...
*Note: not exported by `gt-next` or `gt-react-native`. In `gt-next`, translate strings with [`useGT`](/docs/react/reference/hooks/use-gt) in client components or the server-side `getGT` (documented in the Next.js section).*
