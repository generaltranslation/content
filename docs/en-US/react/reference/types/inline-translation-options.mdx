...
| [`$format`](#format) | The message data format. | `'ICU' \| 'I18NEXT' \| 'STRING'` |
...
Any non-`$` key is treated as an interpolation variable. For the default `ICU` format, reference it in the string with `{name}` syntax; its value is inserted into the translated string without being translated. With `STRING`, variables remain in encoded options but are not substituted into source text.
...
**Type** `'ICU' | 'I18NEXT' | 'STRING'` · **Optional** · **Default** `ICU`

The data format for the message. `ICU` interpolates ICU placeholders and is the default. `I18NEXT` uses i18next-compatible message syntax. `STRING` preserves braces and source text literally without interpolation.
