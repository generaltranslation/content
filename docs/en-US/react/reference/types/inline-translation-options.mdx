...
Any non-`$` key is treated as an interpolation variable. For the default `ICU` format, reference it in the string with `{name}` syntax; its value is inserted into the translated string without being translated. Variables are ignored when `$format` is `STRING`.
...
The data format for the message. `ICU` interpolates variables and is the default. `STRING` preserves braces and source text literally without interpolation.
