...
On-demand development translations run in the browser. Create a **Custom** project key with only **Runtime translation** enabled, then expose it and your project ID through your framework's public environment variables.
...
Create the restricted key under **Project > API Keys** in the [Dashboard](https://dash.generaltranslation.com).

<Callout type="warn">
  **Warning:** New project keys use the `gtx-api-` prefix in every environment. Never expose a key with permissions beyond **Runtime translation**.
</Callout>
...
Set your private build environment variables in your hosting provider:
...
<Callout type="warn">
  **Warning:** Never publicly expose `GT_API_KEY`. Keep build credentials in your hosting provider's secret settings.
</Callout>
