...
On-demand development translations run in the browser. Expose your project ID and an API key to client code using your framework's public environment variables. Because this key ships to the browser, create it with **Custom** permissions and enable only **Runtime translation**.

With Vite, `gt-react` reads these variables automatically:

```bash title=".env.local"
VITE_GT_PROJECT_ID="your-project-id"
VITE_GT_DEV_API_KEY="your-api-key"
```
...
<Callout type="warn">
  **Warning:** Anything in a public environment variable is visible to users. Keep the key you use for [`npx gt translate`](/docs/cli/reference/commands/translate) in a server-side `GT_API_KEY`, never in a public variable.
</Callout>
...
GT_API_KEY=gtx-api-your-api-key
```

<Callout type="warn">
  **Warning:** Never publicly expose your `GT_API_KEY`. Get a key from [dash.generaltranslation.com](https://dash.generaltranslation.com); one key works in every environment.
</Callout>
