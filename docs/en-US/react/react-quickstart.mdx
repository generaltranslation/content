...
For a Shopify Hydrogen storefront, follow the [Shopify Hydrogen Quickstart](/docs/react/hydrogen-quickstart) for the same server-rendered setup in Hydrogen's root route.
...
On-demand development translations run in the browser through your local development server. Use a [project API key](/docs/platform/dashboard/reference/api-keys#create-project-keys) with runtime translation permission. A full-access key works, but we recommend **Custom** permissions with only **Runtime translation** enabled to minimize risk. Expose the key and project ID through your framework's public environment variables only for local development.
...
The `VITE_GT_DEV_API_KEY` setting enables development hot reload and accepts a scoped project key starting with `gtx-api-`.

<Callout type="warn">
  **Warning:** Never include any API key in deployed browser or mobile app bundles, even a runtime-only key. Exclude `VITE_GT_DEV_API_KEY` from production builds.
</Callout>
...
<Callout type="warn">
  **Warning:** Use a separate project key with **Files > Write** and **Translation queue > Enabled** for the CLI. Never publicly expose `GT_API_KEY`.
</Callout>
