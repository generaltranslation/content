...
Call **`initializeGTSPA`** in your entry point, before rendering your app. It takes your config and translation loader, determines the user's locale, and loads the translations for it:
...
// Resolves once the initial translations are loaded
...
<Accordions>
  <Accordion title="Using Create React App?">
    CRA restricts imports to files inside `src/` and may not support top-level `await`, so inline the config and render inside `.then()`:
