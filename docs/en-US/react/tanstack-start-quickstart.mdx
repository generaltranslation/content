...
Create `src/start.ts` and register `gtMiddleware` as global request middleware. Keep TanStack Start's CSRF middleware when you define a custom start instance.
...
In `src/routes/__root.tsx`, call [`initializeGT`](/docs/react/tanstack-start/setup#initialize) at the module level, resolve the locale with [`getLocale`](/docs/react/tanstack-start/reference/functions/get-locale), and load a translations snapshot in the route loader. Pass the `locale` and `translations` to `GTProvider`.
