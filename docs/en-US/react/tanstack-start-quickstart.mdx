...
`gt-tanstack-start` adds automatic internationalization to TanStack Start apps. You initialize General Translation in the root route, resolve the request locale, and hydrate [`GTProvider`](/docs/react/reference/components/gt-provider) from a route loader.
...
Install the packages, create a config file and a translation loader, add request middleware, wire up the root route, mark content for translation, and generate translations.
...
The middleware scopes the locale, region, and internationalization setting to each request so server functions use the correct language. Complete the root-route initialization in the next step before starting the development server.

### 5. Set up the root route

In `src/routes/__root.tsx`, call [`initializeGT`](/docs/react/tanstack-start/setup#initialize) at the module level, resolve the locale with [`getLocale`](/docs/react/tanstack-start/reference/functions/get-locale), and load a translations snapshot in the route loader. Pass the `locale` and `translations` to [`GTProvider`](/docs/react/reference/components/gt-provider).
...
[`getLocale`](/docs/react/tanstack-start/reference/functions/get-locale) reads the request-scoped locale on the server and the initialized browser locale on the client. [`GTProvider`](/docs/react/reference/components/gt-provider) requires both `locale` and `translations`.
...
Wrap JSX in the [`<T>`](/docs/react/reference/components/t) component to translate it in place. Import [`<T>`](/docs/react/reference/components/t) and [`useGT`](/docs/react/reference/hooks/use-gt) from `gt-react` so the CLI detects them when scanning your source.
...
## Optional locale routing [#locale-routing]

Locale routing is opt-in. Before you set `localeRouting` to `true`, configure TanStack Router to match both the unprefixed default-locale URL, such as `/about`, and locale-prefixed URLs, such as `/es/about`.

TanStack Router supports two common approaches:

- **Optional locale parameter.** Add an [optional `/{-$locale}` segment](https://tanstack.com/router/v1/docs/guide/internationalization-i18n#i18n-with-optional-path-parameters) to your file-based or code-based routes. For example, `/{-$locale}/about` matches `/about`, `/es/about`, and `/ja/about`.
- **URL rewrite.** Use TanStack Router's [`rewrite` option](https://tanstack.com/router/v1/docs/guide/internationalization-i18n#url-localization-via-router-rewrite) to map locale-prefixed public URLs to your existing route tree.

Then enable locale routing in `gt.config.json`:
...
`gt-tanstack-start` does not create or restructure your routes. With locale routing enabled, [`gtMiddleware`](/docs/react/tanstack-start/reference/functions/gt-middleware) prioritizes a supported locale in the first path segment. The default locale remains unprefixed, and client locale changes reload at the corresponding pathname. See the [TanStack Start setup guide](/docs/react/tanstack-start/setup#locale-routing) for more details.
