...
<Callout type="info">
  **Note:** The `projectId` and `devApiKey` options enable on-demand translation during development. Use a [project API key](/docs/platform/dashboard/reference/api-keys#create-project-keys) with runtime translation permission. Full-access keys work, but we recommend enabling only **Runtime translation** to minimize risk. In Expo, expose the values with the `EXPO_PUBLIC_` prefix only for local development. Never include any API key in release bundles.
</Callout>
