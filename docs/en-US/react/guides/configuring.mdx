...
<Callout type="info">
  **Changed in v11 (React):** the `gt-react` provider no longer takes `config`, `loadTranslations`, or credentials as props. That setup now lives on the `initializeGTSPA` call; the provider only receives the resolved `locale` and `translations`.
</Callout>
