...
<Tabs items={['React', 'Next.js', 'TanStack Start', 'React Native']}>
  ...
  <Tab value="Next.js">
    Set them in your environment; the plugin reads them automatically. `GT_API_KEY` is used server-side and by the CLI. Set `GT_DEV_API_KEY` for development hot reload, and remove it from production build environments.

    ```bash title=".env.local"
    GT_PROJECT_ID="..."
    GT_DEV_API_KEY="gtx-api-..."
    ```
  </Tab>
