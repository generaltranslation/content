...
<Tabs items={['React', 'Next.js', 'TanStack Start', 'React Native']}>
  <Tab value="React">
    Expose the project ID and a project key limited to **Runtime translation** through your framework's client environment-variable convention, then pass them to [`initializeGT`](/docs/react/reference/config#initialize).
  </Tab>

  <Tab value="Next.js">
    Set them in your environment; the plugin reads them automatically. `GT_API_KEY` is used server-side and by the CLI in CI. `GT_DEV_API_KEY` enables on-demand translation during development. Use the `NEXT_PUBLIC_` prefix only for values that must reach the browser.
...
*Note: New project keys use the `gtx-api-` prefix in every environment. Only expose a **Custom** key with **Runtime translation** enabled to client code. Keep keys with broader permissions on the server or in CI.*
...
In development, providing a [`projectId`](/docs/react/reference/config#project-id) and project key through the development credential setting enables on-demand translation and hot reload, so new strings translate as you work. In production, translations come from your pre-generated files or the CDN.
