...
<Tabs items={['React', 'Next.js', 'TanStack Start', 'React Native']}>
  <Tab value="React">
    ...
    <Callout type="info">
      **Changed in v11:** the `gt-react` provider no longer takes `config`, `loadTranslations`, or credentials as props. That setup now lives on the `initializeGTSPA` call above; the provider only receives the resolved `locale` and `translations`.
    </Callout>
  </Tab>
