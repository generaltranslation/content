...
<Tabs items={['React', 'Next.js', 'TanStack Start', 'React Native']}>
  <Tab value="React">
    Expose the project ID and an API key through your framework's client environment-variable convention, then pass them to [`initializeGT`](/docs/react/reference/config#initialize). Because this key reaches the browser, give it only **Runtime translation** permission.
  </Tab>

  <Tab value="Next.js">
    Set them in your environment; the plugin reads them automatically. `GT_API_KEY` is used server-side and by the CLI in CI, and `GT_DEV_API_KEY` enables on-demand translation in development. The same key can fill both. Use the `NEXT_PUBLIC_` prefix only for values that must reach the browser.
...
*Note: Only expose a key limited to **Runtime translation** to the client. The key the CLI uses in CI is never shipped to the client.*
...
In development, providing a [`projectId`](/docs/react/reference/config#project-id) and a [`devApiKey`](/docs/react/reference/config#initialization) enables on-demand translation and hot reload, so new strings translate as you work. In production, translations come from your pre-generated files or the CDN.
