...
<Tabs items={['React', 'Next.js', 'TanStack Start', 'React Native']}>
  ...
  <Tab value="Next.js">
    Use `NEXT_PUBLIC_` variables to expose your project ID and development key to browser code for local development translation. The SDK reads them automatically. Keep `GT_API_KEY` unprefixed for server and CLI use, and remove `NEXT_PUBLIC_GT_DEV_API_KEY` from production build environments.

    ```bash title=".env.local"
    NEXT_PUBLIC_GT_PROJECT_ID="..."
    NEXT_PUBLIC_GT_DEV_API_KEY="gtx-api-..."
    ```
  </Tab>
