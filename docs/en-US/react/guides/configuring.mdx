...
Translation delivery uses your project ID. For on-demand development translation, use a [project API key](/docs/platform/dashboard/reference/api-keys#create-project-keys) with runtime translation permission. Full-access keys work, but we recommend **Custom** permissions with only **Runtime translation** enabled to minimize risk. Pass it through the SDK's `devApiKey` setting or the development environment variable shown below.

<Tabs items={['React', 'Next.js', 'TanStack Start', 'React Native']}>
  <Tab value="React">
    Expose the project ID and project API key with runtime translation permission through your framework's client environment-variable convention, then pass them to [`initializeGT`](/docs/react/reference/config#initialize). Expose the key only through your local development server, and exclude all API keys from deployed client bundles.
  </Tab>

  <Tab value="Next.js">
    For App Router, use private server variables, matching native CLI setup. The SDK reads them automatically. Keep `GT_API_KEY` unprefixed and separate for server or CLI use.
    ...
    Pages Router browser development uses `NEXT_PUBLIC_GT_PROJECT_ID` and `NEXT_PUBLIC_GT_DEV_API_KEY` instead. Public values win if both variants are set; remove stale duplicates. Exclude both development-key variants from production builds, where `gt-next` rejects them. (See [Next.js credentials](/docs/react/nextjs/config#credentials)).
  </Tab>
  ...
  <Tab value="React Native">
    ...
    EXPO_PUBLIC_GT_DEV_API_KEY="gtx-api-..."
    ```
  </Tab>
</Tabs>

*Note: Never include any API key in deployed browser or mobile app bundles, including runtime-only keys. Client-side keys are for local development only. Keep deployed credentials in server-side environment variables.*
...
In development, providing a [`projectId`](/docs/react/reference/config#project-id) and project API key with runtime translation permission enables on-demand translation and hot reload, so new strings translate as you work. In production, translations come from your pre-generated files or the CDN.
