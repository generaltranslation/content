...
<Tabs items={['React', 'Next.js', 'TanStack Start', 'React Native']}>
  <Tab value="React">
    ...
    The provider also accepts [`region`](/docs/react/reference/components/gt-provider#region) and [`enableI18n`](/docs/react/reference/components/gt-provider#enable-i18n) (default `true`). Disabling translation renders content in the source language. Translation is also skipped when the active locale is the same dialect as the source locale.
  </Tab>
