...
Do not leave the original entry configured as a second browser entry. It must render only after `initializeGTSPA` resolves. Run the repository's normal Bazel build or test target for the application. Do not start a persistent development or preview server.
