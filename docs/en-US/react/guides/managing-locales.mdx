...
- [`getLocaleProperties`](/docs/react/reference/functions/get-locale-properties) reads the same metadata from initialized framework configuration in `gt-react`, `gt-next`, and `gt-react-native`.
