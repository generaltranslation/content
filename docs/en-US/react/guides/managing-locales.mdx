...
- [`getLocaleProperties`](/docs/react/reference/functions/get-locale-properties) reads the same metadata without provider state in `gt-react`, `gt-next`, and `gt-react-native`.
