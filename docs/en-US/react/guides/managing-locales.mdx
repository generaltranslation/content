...
If you only need a direct action, call [`useSetLocale`](/docs/react/reference/hooks/use-set-locale) from your framework's package. This example uses `gt-react`:
...
- **Next.js App Router:** stores the locale in a cookie, then refreshes the server component tree. Locale middleware applies any configured path routing.
...
- **TanStack Start:** stores the locale in a cookie, then reloads the page or navigates to the corresponding locale pathname when [`localeRouting`](/docs/react/reference/config#locale-routing) is enabled.
