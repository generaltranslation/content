...
- **Next.js App Router:** stores the locale in a cookie, then usually refreshes the server component tree. Locale middleware applies any configured path routing. When routing removes a non-default locale prefix to return to the unprefixed default locale, the browser reloads so it can follow the middleware redirect.
