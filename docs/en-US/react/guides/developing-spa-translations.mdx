...
Development translations let you preview translated content as you edit your SPA. They require the GT compiler and an API key exposed to the browser through `VITE_GT_DEV_API_KEY` or your bundler's equivalent.

**Prerequisites:**

- A single-page React app configured with the [SPA Quickstart](/docs/react/react-spa-quickstart)
- A project API key with only **Runtime translation** enabled, because the key is exposed to browser code
...
Create an API key at [dash.generaltranslation.com](https://dash.generaltranslation.com/en-US/signin) or by running:

```bash
npx gt auth
```

Then add your project ID and API key to `.env.local` and pass them to your initialization function:
...
**Browser-exposed key:** This key ships to the browser. Create it with **Custom** permissions and only **Runtime translation** enabled, and never expose the key you use for [`npx gt translate`](/docs/cli/reference/commands/translate).
