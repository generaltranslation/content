...
The compiler registers source changes and requests updated translations while your development server is running.

## Before you start [#before-start]

- A single-page React app configured with the [SPA Quickstart](/docs/react/react-spa-quickstart)
- A **Custom** project key with **Runtime translation** enabled
...
Create a **Custom** project key with only **Runtime translation** enabled under **Project > API Keys** in the Dashboard. New project keys use the `gtx-api-` prefix in every environment.

Then add your project ID and key to `.env.local` and pass them to your initialization function:
...
**Development only:** expose only a **Custom** key with **Runtime translation** enabled. Never expose a key with broader permissions in browser code.
