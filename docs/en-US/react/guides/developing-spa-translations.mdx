...
Development translations let you preview translated content as you edit your SPA. They require the GT compiler and a project API key with runtime translation permission.

**Prerequisites:**

- A single-page React app configured with the [SPA Quickstart](/docs/react/react-spa-quickstart)
- A [project API key](/docs/platform/dashboard/reference/api-keys#create-project-keys) starting with `gtx-api-`, with **Runtime translation** enabled
...
In **Project > API Keys**, create a key with **Runtime translation** enabled. Full-access keys also work. To minimize risk, we recommend **Custom** permissions with **Runtime translation** set to **Enabled** and every other resource set to **None**.

Add your project ID and project key to `.env.local`, then pass them to your initialization function. The SDK still uses the `devApiKey` setting to enable development hot reload:
...
**Development only:** API keys may be used in client code served by your local development server. Never include any API key in deployed client bundles, regardless of its permissions. Restricting local development keys to **Runtime translation** is recommended to minimize risk.
