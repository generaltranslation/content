...
description: Bundle General Translation translations in your app instead of loading them from the CDN, across React, Next.js, TanStack Start, and React Native.
...
Provide a `loadTranslations` callback that imports the file for a locale. How you register it depends on the framework.
...
With a local loader in place, translations are read from your bundle and no CDN request is made. To switch back to CDN delivery, remove the loader and provide a `projectId` instead (see [Configure General Translation](/docs/react/guides/configure#delivery)).
