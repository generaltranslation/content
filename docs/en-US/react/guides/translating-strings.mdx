...
Some text is not JSX — placeholders, `aria-label`s, alt text, and strings you build in code. Use the [`useGT`](/docs/react/reference/hooks/use-gt) hook to translate these. In async App Router components, use [`getGT`](/docs/react/nextjs/server-functions#get-gt) instead.
