...
related:
  links:
    ...
    - /docs/react/reference/functions/msg
    - /docs/react/reference/hooks/use-messages
---
...
To define translatable strings at module scope — for constants, enums, or config — register them with [`msg`](/docs/react/reference/functions/msg) and resolve them with [`useMessages`](/docs/react/reference/hooks/use-messages). Register the string once with `msg`, then resolve it where you render.
...
<Tabs items={['React', 'Next.js', 'TanStack Start', 'React Native']}>
  ...
  <Tab value="Next.js">
    ...
    In server components, register with `msg` from `gt-next` and resolve with the async [`getMessages`](/docs/node/reference/functions/get-messages) from `gt-next/server`:
