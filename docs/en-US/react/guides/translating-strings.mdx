...
Some text is not JSX — placeholders, `aria-label`s, alt text, and strings you build in code. Use the [`useGT`](/docs/react/reference/hooks/use-gt) hook to translate these. In async Next.js components, use [`getGT`](/docs/react/nextjs/server-functions#get-gt) instead.
...
To define translatable strings at module scope — for constants, enums, or config — register them with [`msg`](/docs/react/reference/functions/msg) and resolve them with [`useMessages`](/docs/react/reference/hooks/use-messages). In async Next.js components, use [`getMessages`](/docs/react/nextjs/server-functions#get-messages) instead. Register the string once with `msg`, then resolve it where you render.
