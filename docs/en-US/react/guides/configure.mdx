...
<Tabs items={['React', 'Next.js', 'TanStack Start', 'React Native']}>
  <Tab value="React">
    Call an initialization function once. Use `initializeGTSPA` for client-rendered apps such as Vite — it detects the locale from cookies and the browser and preloads translations. Use `initializeGT` when a server or framework layer provides translations instead.
    ...
    The `loadTranslations` and `loadDictionary` callbacks, credentials, and locale configuration all go on the initialization call — not on `GTProvider`.
  </Tab>
...
<Tabs items={['React', 'Next.js', 'TanStack Start', 'React Native']}>
  ...
  <Tab value="Next.js">
    Wrap your root layout in `<GTProvider>`. It reads the request locale and translations from the plugin, so it needs no `locale` or `translations` props.
...
- **Local files:** provide `loadTranslations` to import bundled JSON. See [Store translations locally](/docs/react/guides/local-translations).
