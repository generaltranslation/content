...
- **`getLocale`** — returns the locale from the active middleware scope on the server and from the initialized browser condition store on the client.
...
First configure your TanStack Router route tree to accept locale-prefixed paths. Then set `localeRouting` to `true` in `gt.config.json` to keep the locale in the pathname:
...
The default locale remains unprefixed, such as `/about`. Other locales use a prefix, such as `/es/about`. The package resolves and changes the pathname but does not define your application routes, so both URL shapes must match your route tree.

On the server, `gtMiddleware` resolves the locale from the path, then the locale cookie, the `Accept-Language` header, and `defaultLocale`. On the client, locale changes reload at the corresponding pathname and preserve its query string and hash.
