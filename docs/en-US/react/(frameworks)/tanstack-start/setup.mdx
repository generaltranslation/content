...
- **[`initializeGT`](/docs/react/reference/config#initialize)** — call once at the module level of the root route. Spread in your `gt.config.json` and pass [`loadTranslations`](/docs/react/reference/functions/load-translations).
...
In `src/routes/__root.tsx`, initialize at the module level, resolve the locale and load the snapshot in the loader, then pass `locale` and `translations` to [`<GTProvider>`](/docs/react/reference/components/gt-provider) in the shell component.
...
[`initializeGT`](/docs/react/reference/config#initialize) must run before [`gtMiddleware`](/docs/react/tanstack-start/reference/functions/gt-middleware) handles a request. Finish this root-route setup before starting the development server.
