...
By default, FormatJS checks whether each API is already supported before installing its JavaScript implementation. Add selected APIs to `forcePolyfills` to bypass that check when profiling shows it contributes to cold-start time. Forcing a polyfill always installs its JavaScript implementation, so enable it only for the APIs that benefit in your app.
...
| [`forcePolyfills`](#force-polyfills) | Polyfills that should bypass runtime capability detection. | `ForceablePolyfill[]` | Yes | `[]` |
...
### `forcePolyfills` [#force-polyfills]

**Type** `ForceablePolyfill[]` · **Optional** · **Default** `[]`

Base polyfill import paths that should use FormatJS's `polyfill-force` entry point instead of checking the runtime first. Supported values are:

- `@formatjs/intl-displaynames/polyfill`
- `@formatjs/intl-listformat/polyfill`
- `@formatjs/intl-relativetimeformat/polyfill`

For example, force only `Intl.DisplayNames` and `Intl.ListFormat`:
...
Use the normal base import paths in this option; the plugin maps them to the corresponding forced imports. `excludePolyfills` takes precedence if the same polyfill appears in both arrays. This option does not change which locale-data modules are added; configure those with `locales`.

<Callout type="warn">
  **Performance tradeoff:** A forced polyfill replaces a runtime's native
  implementation with FormatJS's JavaScript implementation. Use this option
  selectively when profiling shows that capability detection is slower than
