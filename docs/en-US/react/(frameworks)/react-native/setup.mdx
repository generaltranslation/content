...
*Note: in Expo, public environment variables must be prefixed with `EXPO_PUBLIC_`. The key in `EXPO_PUBLIC_GT_DEV_API_KEY` ships in your app bundle, so give it only **Runtime translation** permission.*
