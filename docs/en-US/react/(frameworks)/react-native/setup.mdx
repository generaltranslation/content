...
*Note: in Expo, public environment variables must be prefixed with `EXPO_PUBLIC_`. Full-access project keys work for local development, but we recommend a key with only **Runtime translation** enabled to minimize risk. Never include any API key in release bundles, regardless of its permissions.*
