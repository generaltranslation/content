...
Wrap your app in `<GTProvider>`. In React Native, the provider loads the active locale's translations internally and manages locale state, so it does not require a `translations` prop and resolves the `locale` from the device by default (you can still pass an optional `locale` override — see below). It renders a loading fallback while translations load; pass a `fallback` prop to customize it.
