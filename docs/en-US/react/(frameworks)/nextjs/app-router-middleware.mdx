...
<Callout type="info">
  **Next.js 15.1–15.4:** `next/root-params` is available in Next.js 15.5 and
  newer. On earlier versions, import `unstable_rootParams` from `next/server`
  and return `(await unstable_rootParams())?.locale`.
</Callout>
