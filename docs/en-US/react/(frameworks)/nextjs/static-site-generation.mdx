...
description: How to pre-render localized Next.js App Router and Pages Router pages with General Translation
related:
  links:
...
**Note:** Review these common issues when setting up static site generation:
...
## Pages Router setup [#pages-router]
...
### 3. Configure GTProvider navigation
...
<Callout type='info'>
  **Note:** Pages such as `pages/products/[id].tsx` still require `getStaticPaths` for their dynamic parameters. Next.js generates each returned path once per configured locale.
</Callout>

## App Router setup [#app-router]

### Setup requirements

To enable SSG with GT, you need:

1. **App Router with middleware routing** - see [middleware guide](/docs/react/nextjs/middleware)
...
Set up middleware for dynamic requests (see [middleware guide](/docs/react/nextjs/middleware)):
...
### 2. Define locale and region detection
...
### 4. Configure generateStaticParams
...
### 5. Move any layout files inside of the `/[locale]` directory
...
Double-check that [step 2](#2-define-locale-and-region-detection) and [step 3](#3-disable-getregion) are complete.
...
<Callout type="info">
  **Note:** During static site generation, the only way to resolve a user's locale is through the URL path.
