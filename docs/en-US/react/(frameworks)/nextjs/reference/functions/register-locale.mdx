...
The `registerLocale` function sets the locale for the current App Router request context. Use it in Route Handlers and metadata image handlers where `next/root-params` is unavailable.
