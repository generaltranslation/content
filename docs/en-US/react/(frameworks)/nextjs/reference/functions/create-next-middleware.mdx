...
| [`localeRoutes`](#locale-routes) | Limit the shared paths available in each locale. | `LocaleRoutes` | Yes | `{}` |
...
**Type** `LocaleRoutes` · **Optional** · **Default** `{}`

Lists the shared paths available for each locale. Each key is a locale, and its value is an array of shared route patterns. Locales omitted from this option can serve every route. An empty array prevents a locale from serving any routes.
...
In this example, visitors using `en-GB` can view the pricing page and the blog, including its child routes. `/en-GB/prices` still uses the localized alias from `pathConfig` and the locale-specific page implementation from `routeOverrides`. Visitors using French fall back to the default locale for every route, while locales not listed in `localeRoutes` remain unrestricted.

`localeRoutes` always uses shared paths, not localized aliases. It controls routing availability only; it does not check whether content exists in your CMS or other data source. When a requested route is unavailable, the middleware redirects to the corresponding default-locale public URL, preserving configured aliases, route overrides, route parameters, query strings, base paths, and trailing-slash style. The default locale is the final fallback, and a missing default-locale page uses your application's normal Next.js 404 handling.
