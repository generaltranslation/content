...
## Install and initialize your app

Install packages and initialize the library in your entry point.
...
Create a minimal setup. `initializeGTSPA` runs once at startup and resolves when the initial translations are loaded, so await it before rendering.
...
<Tabs items={["Vite (dev)", "Create React App (dev)", "Get keys"]}>
  ...
  `gt-react` reads these automatically during initialization. The dev API key is only read in development.
  </Tab>
