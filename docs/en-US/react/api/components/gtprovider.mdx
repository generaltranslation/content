...
<TypeTable
  type={{
    ...
    'htmlTagOptions?': {
      type: 'HtmlTagOptions',
      optional: true,
      default: '{ updateHtmlLangTag: true, updateHtmlDirTag: true }',
    },
  }}
/>
...
| `htmlTagOptions?` | Controls automatic updates to the `<html>` element's `lang` and `dir` attributes when the locale changes. Both are enabled by default.                                                                                |
