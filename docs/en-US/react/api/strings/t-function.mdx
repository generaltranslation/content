...
<Callout>
**Client-side only:** `t()` is designed for browser environments. If called on the server, it logs a warning and returns the source string.
</Callout>
...
`t()` depends on `bootstrap()` to load translations before your app renders. You need to change the entry point of your Vite app to run `bootstrap()` first.

In a default Vite project, the entry point is `main.tsx`. You'll create a new file (e.g., `bootstrap.tsx`) and point your `index.html` to it instead.
...
```html
<!-- Change the script src to your new bootstrap file -->
...
- Vite only.
- Does not integrate with `<GTProvider>` or other `gt-react` context-based features yet.
