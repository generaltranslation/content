...
Use this quickstart for TanStack Start apps. For a plain React SPA use the [React Quickstart](/docs/react/get-started/react-quickstart).
...
Install `gt-tanstack-start` and `gt-react` as dependencies, and the [`gt` CLI](/docs/cli/quickstart) as a dev dependency. `gt-react` is required directly so the CLI can detect `<T>` components in your source.
...
In `src/routes/__root.tsx`, call `initializeGT` at the module level, resolve the locale with `parseLocale`, and load a translations snapshot in the route loader. Pass the `locale` and `translations` to `GTProvider`.
...
Wrap JSX in the `<T>` component to translate it in place. Import `<T>` and `useGT` from `gt-react` so the CLI detects them when scanning your source.
