...
This quickstart covers both Expo and the bare React Native CLI. For web React apps use the [React Quickstart](/docs/react/get-started/react-quickstart).
...
Call `initializeGT` once at your app's entry point, before rendering, then wrap your app in `GTProvider`. Unlike the web packages, `GTProvider` loads translations internally, so it takes no `locale` or `translations` props.
...
Wrap JSX in the `<T>` component to translate it in place. For standalone strings such as `placeholder` and `accessibilityLabel` values, use the `useGT` hook. To build a language switcher, use the `useLocaleSelector` hook — `gt-react-native` does not ship a prebuilt selector component.
