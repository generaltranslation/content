...
The most robust pattern is a small entry module that initializes General Translation first, then loads the rest of the app. This allows you to translate content at the module level.
...
CommonJS does not support top-level await. Wrap initialization in an async startup function and dynamically import the application afterward. This preserves the async boundary required for module-level `t()` calls.
...
<Callout type="warn">
  **Initialization order:** Do not require `main` before initialization. Requiring it evaluates module-level `t()` calls before translations are ready.
</Callout>
...
`initializeGTSPA` runs once at startup — the configuration is immutable for the lifetime of the app. After it resolves, translations can be resolved in any module. **You don't need to wrap your app in a provider.**
...
<Callout type="warn">
  **Keep your key private:** Do not commit `.env.local` or expose `GT_API_KEY` in browser code.
</Callout>
