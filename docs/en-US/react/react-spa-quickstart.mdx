...
<Callout type="warn">
  **Warning:** Keep `.env.local` out of version control. For CI, use a separately scoped `GT_API_KEY` in your secret store. Never expose tooling keys in browser code. (See [CLI credentials](/docs/cli/guides/configuring#credentials)).
</Callout>
