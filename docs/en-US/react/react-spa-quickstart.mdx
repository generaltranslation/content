...
Login does not select a project or write environment files. Set your existing project's ID in the environment before translating, or add [`projectId`](/docs/cli/reference/config#project-id) to the config from Step 2:

```bash title=".env.local"
GT_PROJECT_ID="your-project-id"
```

This guide already configured your app manually; do not rerun the setup wizard merely to authenticate. If you need guided project selection or creation, use the separate [`gt init`](/docs/cli/reference/commands/init) flow.

<Callout type="warn">
  **Warning:** Keep `.env.local` out of version control. For CI, use a separately scoped `GT_API_KEY` in your secret store. Explicit tooling keys override login, including keys loaded from env files; never expose them in browser code. (See [CLI credentials](/docs/cli/guides/configuring#credentials)).
</Callout>
