...
Follow the prompts to create an account or log in. When prompted for a key type, choose `production` so the CLI writes the project key to `GT_API_KEY`. The command generates an API key and project ID, then adds them to `.env.local` in your project root:
