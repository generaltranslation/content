...
The command opens a browser or device-code flow and stores the login outside your project. Copy your project ID from the Dashboard and add it to `.env.local`:
...
<Callout type="info">
  **CI:** Set `GT_PROJECT_ID` and a project key in `GT_API_KEY` as secret environment variables. Do not commit the key or expose it to browser code.
</Callout>
