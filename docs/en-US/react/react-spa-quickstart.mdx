...
Follow the prompts to create an account or log in. When asked which key type to generate, choose the production option so the key is written as `GT_API_KEY`. The command generates an API key and project ID, then adds them to `.env.local` in your project root:
