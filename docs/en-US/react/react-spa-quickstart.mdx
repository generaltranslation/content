...
Follow the prompts to create an account or log in. The command generates one project API key that works in every environment and writes it to the `GT_API_KEY` slot in `.env.local`:
