...
`jest-environment-jsdom` uses an older version of jsdom that doesn't include the `TextEncoder` global. GT libraries depend on it internally for hashing. The current version of jsdom has added support, but Jest hasn't updated to use it yet.

The workaround is to polyfill it in a setup file:
