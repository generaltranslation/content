...
The same applies to dictionaries: if a target-locale translation for an entry is missing, `useTranslations` falls back to the source entry. However, if the entry itself is missing from the source dictionary, `useTranslations` throws an error (in both development and production).
