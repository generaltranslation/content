...
The root package exports the supported [`GT`](/docs/platform/core/reference/gt-class/constructor) class for scripts and server tooling. Use [`generaltranslation/runtime`](/docs/platform/core/reference/runtime) for runtime translation, formatting, and locales without file/project management, or [`generaltranslation/api`](/docs/platform/core/reference/api-client) for generated endpoint helpers. The runtime entry is a bundle boundary, not permission to expose API keys in browser or mobile bundles.
...
Translation methods require a project ID and either an API key or an application-owned [token provider](/docs/platform/core/reference/api-client#usertokenprovider). The SDK does not load CLI login sessions. Formatting and locale utilities work without translation API access.