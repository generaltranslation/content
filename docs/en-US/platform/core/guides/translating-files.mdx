...
For structured files such as JSON, YAML, HTML, Android string resources, gettext, and Apple string files, the platform may compare translated units within the file for inconsistent terminology. A unit can be a plain string, plural, or format-specific variation. Chunked formats such as Markdown, MDX, TypeScript, and plain text use a similar terminology pass across chunks. Each correction must preserve the format's placeholders and structure; when a correction is invalid or cannot complete, the first-pass translation is kept.
