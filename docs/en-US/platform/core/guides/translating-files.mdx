...
For SVG files, pass the UTF-8 XML source with `fileFormat: 'SVG'`. Translation updates text nodes while preserving shapes and styles. An asynchronous layout pass can recenter text and shorten overflowing translations while preserving authored font sizes, so wait until the file is complete before downloading it. Malformed XML is rejected.
...
## 2. Enqueue the file for translation [#enqueue]
...
The result contains job information. The wrapper synthesizes `locales` and the summary `message`; do not depend on the message's exact wording.
...
Wait for jobs with a bounded [`awaitJobs`](/docs/platform/core/reference/gt-class-methods/translation/await-jobs) deadline, then use [queryFileData](/docs/platform/core/reference/gt-class-methods/translation/query-file-data) to confirm the requested version is ready. Stop on incomplete, failed, or unknown jobs; an empty job list can mean no new work was needed.
...
const translatedFile = status.translatedFiles?.[0];
if (!translatedFile || translatedFile.completedAt === null) {
  throw new Error('The requested translation is not ready to download');
}
```
...
Text formats return decoded UTF-8; binary `LOTTIE` content remains base64. An empty successful download response returns `''`, which alone does not establish success. This JSON example returns:
