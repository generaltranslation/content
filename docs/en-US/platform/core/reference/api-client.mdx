...
The bundled `@generaltranslation/api` 0.1.0 SDK exports these generated operation helpers:
...
[`awaitJobs`](#job-polling) uses a configured client to call `getTranslationJobInfo` until every requested job is `completed`, `failed`, or `unknown`, or until the overall timeout expires. A requested ID omitted from the response is reported as `unknown`.
...
`AwaitJobsResult` contains `complete`, which is `false` when the deadline is reached before every requested job is resolved, and `jobs`, which contains the latest result for every requested ID. Passing an empty array resolves immediately with `{ complete: true, jobs: [] }`.
