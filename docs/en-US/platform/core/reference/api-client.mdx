...
`generaltranslation/api` is available in `generaltranslation` 9.2.0 and later. The examples below target `generaltranslation` 9.4.0 and the equivalent standalone `@generaltranslation/api` 0.3.0 package.
...
The installed 0.3.0 SDK exports these generated operation helpers:

| Function | Operation |
| --- | --- |
| [`createProject`](/docs/platform/openapi/reference/project/create-project) | Create a project in a selected Organization. |
| [`createProjectApiKey`](/docs/platform/openapi/reference/project/create-api-key) | Create a production or development API key for a project. |
...
| `workspacePluginInfo` | Read Google Drive integration and linked-file context. |
| `workspacePluginTranslate` | Translate a Google Docs or Google Slides file. |
| `workspacePluginStatus` | Read Google Drive translation and layout progress. |
| `figmaPluginInfo` | Read Figma plugin project information. |
| `figmaPluginSync` | Synchronize Figma plugin content. |
| `figmaPluginImportTranslations` | Import translations from Figma. |
| `figmaPluginEnqueue` | Queue Figma content for translation. |
| `figmaPluginStatus` | Read Figma translation status. |
| `figmaPluginDownload` | Download translated Figma content. |
...
The module exports `Client`, `Options`, `ApiClientConfig`, `ApiVersion`, `RetryPolicy`, `AwaitJobsOptions`, `AwaitJobsResult`, `GetJobStatuses`, `JobResult`, `BatchOptions`, `RuntimeFileFormat`, and all generated OpenAPI types.
