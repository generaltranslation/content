...
The installed 0.2.0 SDK exports these generated operation helpers:
...
<Callout type="warn">
  The 0.2.0 `createProject` helper calls the removed `POST /v2/projects` route and cannot create projects against the current hosted API. Use the [Create a project](/docs/platform/openapi/reference/project/create-project) endpoint directly until a compatible SDK release is available.
</Callout>

Use the [public OpenAPI reference](/docs/platform/openapi/overview) for live endpoint permissions, rate limits, schemas, and status codes. The current public contract also includes project API-key creation, which is not yet exported as a generated helper in SDK 0.2.0.
