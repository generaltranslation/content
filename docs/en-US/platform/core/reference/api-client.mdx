...
The SDK neither implements OAuth login nor reads sessions saved by [`gt login`](/docs/cli/reference/commands/login). Your application owns the token lifecycle:
...
import { createApiClient, listProjects } from 'generaltranslation/api';
...
import { createApiClient, createProjectApiKey } from 'generaltranslation/api';
...
Omitting `permissions` requests all delegable project permissions held by the caller. Every selected permission must be held; duplicates are ignored. The legacy key `type` input is deprecated and ignored. These generated discovery and creation helpers are not methods on [`GT`](/docs/platform/core/reference/gt-class/constructor). (See [Create project API key](/docs/platform/openapi/reference/project/create-api-key) for grants and provisioning constraints).
