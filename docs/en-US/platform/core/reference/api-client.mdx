...
The installed 0.4.0 SDK provides generated helpers for these 23 public operations:
...
Project discovery returns one page of projects readable by your API key, not all results. Follow `nextCursor` to collect every project.
...
Project creation takes `path: { orgId }` and `body: { name, defaultLocale, cdnEnabled? }`.
