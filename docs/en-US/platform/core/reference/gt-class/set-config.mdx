...
- **Configured identities.** Locale spellings are preserved. All provided locale fields, including `locales`, validate against the effective custom mapping. Invalid codes throw. Service requests canonicalize locale codes; response recovery follows the [constructor rules](/docs/platform/core/reference/gt-class/constructor#how-it-works).
- **Client lifetime.** This is the supported way to switch credentials or project settings. It invalidates the cached management client even when validation throws after credentials changed. Direct property assignment does not rebuild that client.
