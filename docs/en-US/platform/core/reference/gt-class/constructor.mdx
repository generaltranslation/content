...
| --- | --- | --- | --- | --- |
| [`apiKey`](#api-key) | Project API key for production runtime translation. | `string` | Yes | `GT_API_KEY` env |
| [`devApiKey`](#dev-api-key) | Project API key for on-demand development translation. | `string` | Yes | `GT_DEV_API_KEY` env |
...
Project API key for production runtime translation. Read from the `GT_API_KEY` environment variable when not provided. Required for any API operation.

### `devApiKey` [#dev-api-key]

**Type** `string` · **Optional** · **Default** `GT_DEV_API_KEY` env

Project API key for on-demand development translation. Takes precedence over `apiKey` in development. Read from the `GT_DEV_API_KEY` environment variable when not provided.
