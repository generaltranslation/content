...
Call `formatRelativeTimeFromDate` on a `GT` instance with the target `Date` and an optional options object. It returns the formatted string, choosing the unit that best fits the time difference.
