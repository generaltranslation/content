...
- **Locale resolution.** By default the method formats for the instance's target locale, falling back to the source locale and then the library default (`en`) — not the `locales` config array. Pass `locales` in the options to override for a single call.
- **Intl-backed.** Formatting is delegated to the current runtime's [`Intl.NumberFormat`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/NumberFormat), so all standard `Intl.NumberFormatOptions` supported by that runtime are available and locale conventions are applied automatically.
...
- The method uses the current runtime's `Intl.NumberFormat` for performance and accuracy.
