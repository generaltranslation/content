...
- **Intl-backed.** Formatting is delegated to the current runtime's [`Intl.DateTimeFormat`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/DateTimeFormat), so all standard `Intl.DateTimeFormatOptions` supported by that runtime are available.
...
- The method uses the current runtime's `Intl.DateTimeFormat` for performance and accuracy.
