...
Call `formatMessage` on a `GT` instance with a message string and an optional options object holding the `variables` to interpolate. It returns the formatted message.
...
*Note: `formatMessage` runs locally using `Intl.MessageFormat` and does not require an API key. It overrides the instance's locales when `locales` is provided. For formatting without a `GT` instance, see the standalone [`formatMessage`](/docs/platform/core/reference/utility-functions/formatting/format-message).*

## How it works [#how-it-works]

- **ICU processing.** The method parses ICU message format syntax with `Intl.MessageFormat` from Format.JS and applies locale-specific number, date, and currency formatting automatically.
...
- **Escaping braces.** Double curly braces escape the interpolation behavior and render as a single curly brace.
...
- Double curly braces: escape the curly-brace behavior and render as a single curly brace.
...
- The method processes ICU message format syntax with `Intl.MessageFormat` from Format.JS.
