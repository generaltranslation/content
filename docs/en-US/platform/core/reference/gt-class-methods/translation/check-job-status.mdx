...
*Note: `checkJobStatus` requires an API key (including the deprecated `devApiKey` alias) or a [token provider](/docs/platform/core/reference/api-client#usertokenprovider), plus `projectId` on the GT instance. To check a setup job, first call both [`uploadSourceFiles`](/docs/platform/core/reference/gt-class-methods/translation/upload-source-files) and [`setupProject`](/docs/platform/core/reference/gt-class-methods/translation/setup-project).*
...
For either translation or setup jobs, use [`awaitJobs`](/docs/platform/core/reference/gt-class-methods/translation/await-jobs) instead of manual polling. Pass an array containing the setup job ID.
