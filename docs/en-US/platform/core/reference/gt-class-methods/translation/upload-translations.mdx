...
*Note: `uploadTranslations` requires an API key (including the deprecated `devApiKey` alias) or a [token provider](/docs/platform/core/reference/api-client#usertokenprovider), plus `projectId` on the GT instance.*

## How it works [#how-it-works]

- **Existing source.** Upload the source first. Supply a complete `FileUpload`, including content and locale, matching that source version. File and version IDs are optional.
...
| `transformFormat` | Not used for the source file in this upload. | [`FileFormat`](/docs/platform/core/reference/types/file-format) | Yes |
| `incomingBranchId` | Optional branch metadata accepted for the source file. | `string` | Yes |
| `checkedOutBranchId` | Optional branch metadata accepted for the source file. | `string` | Yes |

Use the supported fields listed here. This method uploads translations; including source metadata does not update the existing source file.
...
| `fileId` | Ignored; the translation uses the source file ID. | `string` | Yes |
| `versionId` | Ignored; the translation uses the source version ID. | `string` | Yes |
| `branchId` | Ignored; the translation uses the source branch. | `string` | Yes |
| `transformFormat` | Optional format accepted for translation uploads. | [`FileFormat`](/docs/platform/core/reference/types/file-format) | Yes |
| `formatMetadata` | Not supported for translated files. | `GTJsonFormatMetadata \| FormatMetadata` | Yes |
| `incomingBranchId` | Not supported for translated files. | `string` | Yes |
| `checkedOutBranchId` | Not supported for translated files. | `string` | Yes |
...
type UploadFilesResponse = {
  ...
  count: number; // number of uploaded references
  message: string; // human-readable summary; wording may change
};
```
