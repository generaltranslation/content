...
| `fileFormat` | Format of the file. One of `GTJSON`, `JSON`, `PO`, `POT`, `YAML`, `MDX`, `MD`, `TS`, `JS`, `HTML`, `TXT`, `TWILIO_CONTENT_JSON`, `LOTTIE`, or `SVG`. | [`FileFormat`](/docs/platform/core/reference/types/file-format) | No |
...
| Field | Description | Type | Optional |
| --- | --- | --- | --- |
| `content` | Raw translated text, including SVG XML, or base64-encoded binary content for `LOTTIE`. | `string` | No |
...
| `modelProvider` | AI model provider preference: `ANTHROPIC`, `OPENAI`, `XAI`, or `GOOGLE`. | `string` | Yes |
