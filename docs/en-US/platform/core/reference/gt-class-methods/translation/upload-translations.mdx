...
| `incomingBranchId` | Serialized and declared in the source endpoint input. | `string` | Yes |
| `checkedOutBranchId` | Serialized and declared in the source endpoint input. | `string` | Yes |

Both source and translation objects are serialized with encoded content and canonicalized locales. A field being sent does not imply it updates an existing source record or is supported by the endpoint.
...
| `transformFormat` | Serialized and declared in the translation endpoint input. | [`FileFormat`](/docs/platform/core/reference/types/file-format) | Yes |
| `formatMetadata` | Serialized from the class input, but not a supported translation endpoint field. | `GTJsonFormatMetadata \| FormatMetadata` | Yes |
| `incomingBranchId` | Serialized from the class input, but not a supported translation endpoint field. | `string` | Yes |
| `checkedOutBranchId` | Serialized from the class input, but not a supported translation endpoint field. | `string` | Yes |
