...
- **Up to one job per file and locale.** The response `jobData` is keyed by job ID. A source-file and target-locale pair that is already complete does not create a new job unless forced.
