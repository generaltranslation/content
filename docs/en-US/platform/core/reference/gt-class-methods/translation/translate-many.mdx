...
*Note: `translateMany` requires an API key (including the deprecated `devApiKey` alias) or a [token provider](/docs/platform/core/reference/api-client#usertokenprovider), plus `projectId` on the GT instance.*

## How it works [#how-it-works]

- **Array vs. record.** Array results follow input order. For a record, use entry hashes as keys; results retain those keys.
...
- **Whole documents.** Set [`metadata.fileFormat`](/docs/platform/core/reference/types/entry-metadata#file-format) to `'MD'` or `'MDX'` to parse and translate a complete document while preserving its structure. The document must be a string with `dataFormat: 'STRING'`, which is the default, and cannot use `maxChars`. If translation fails or the resulting document is invalid, the entry fails rather than returning partial output.
...
Narrow each result on `success` before reading its translation. Runtime translation disables transport retries, including `429`; token-provider `401` replay remains separate. Configuration, authentication, HTTP, network, cancellation, and timeout failures can reject the whole batch. Results retain the service locale rather than restoring configured aliases. (See [Errors](/docs/platform/core/reference/api-client#types-errors)).
...
```typescript
// Translate a complete MDX document.
const [result] = await gt.translateMany(
  [
    {
      source: '# Welcome\n\n<Callout>Read the guide.</Callout>',
...
const greeting = results['greeting-hash'];
if (greeting?.success) {
  console.log(greeting.translation);
}
```
