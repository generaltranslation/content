...
Call `translateMany` on a configured `GT` instance with a collection of entries and either a target locale string (shorthand) or an options object. It accepts the entries as an array or as a record keyed by hash, and returns results in the matching shape.
