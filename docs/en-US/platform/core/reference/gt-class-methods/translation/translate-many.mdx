...
*Note: `translateMany` requires an API key (including the deprecated `devApiKey` alias), plus `projectId` on the GT instance.*
...
Narrow each result on `success` before reading its translation. Runtime translation disables transport retries, including `429`. Configuration, authentication, HTTP, network, cancellation, and timeout failures can reject the whole batch. Results retain the service locale rather than restoring configured aliases. (See [Errors](/docs/platform/core/reference/api-client#types-errors)).
