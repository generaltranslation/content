...
- **Locale resolution.** Translation queries canonicalize locales at the service boundary. Response locales recover [configured identities](/docs/platform/core/reference/gt-class/constructor#how-it-works); the first matching identity wins when aliases are ambiguous. Do not expect a distinct response label for every equivalent requested spelling.
