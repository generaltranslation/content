...
*Note: `querySourceFile` requires an API key (including the deprecated `devApiKey` alias) or a [token provider](/docs/platform/core/reference/api-client#usertokenprovider), plus `projectId` on the GT instance.*
...
- **Configured locales.** The `locales` array on the source file lists all target locales configured for translation. Returned locale codes recover [configured identities](/docs/platform/core/reference/gt-class/constructor#how-it-works) where possible; the first matching identity wins when aliases are ambiguous.
...
| --- | --- | --- |
| `id` | Unique source file record identifier. | `string` |
