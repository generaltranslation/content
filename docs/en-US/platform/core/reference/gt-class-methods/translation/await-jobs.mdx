...
Polls the status of translation jobs and resolves once every job reaches a terminal state (`completed`, `failed`, or `unknown`) or a timeout is reached. It uses the shared sequential [polling loop](/docs/platform/core/reference/api-client#job-polling) for translation and setup jobs.
...
- **Deadline.** Polls do not overlap and request only pending IDs. Each status request receives an abort signal capped at the smaller of the remaining deadline and 60 seconds. Late results are discarded. At timeout, the last accepted statuses return with `complete: false`; they can still be queued, processing, or initially unknown. A fetch implementation that ignores the signal can delay resolution.
