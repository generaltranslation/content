...
- **File encoding.** Text content is automatically base64-encoded for safe transmission. Binary `LOTTIE` and `DOT_STRINGS` content must already be base64-encoded.
...
| --- | --- | --- | --- |
| `content` | Raw text content, or base64-encoded binary content for `LOTTIE` and `DOT_STRINGS`. | `string` | No |
...
- Text content is automatically base64-encoded for safe transmission. Pass `LOTTIE` and `DOT_STRINGS` content as base64-encoded binary data.
