...
The API returns the resolved `branchId`, including the project default when the upload did not specify a branch. It does not populate `transformFormat`; set that field before passing a reference to [`enqueueFiles`](/docs/platform/core/reference/gt-class-methods/translation/enqueue-files) when you need format conversion.
