...
- Accepted file-format identifiers are `GTJSON`, `JSON`, `PO`, `POT`, `YAML`, `MDX`, `MD`, `TS`, `JS`, `HTML`, `TXT`, `TWILIO_CONTENT_JSON`, `LOTTIE`, and `SVG` — see [`FileFormat`](/docs/platform/core/reference/types/file-format) for format-specific availability.
