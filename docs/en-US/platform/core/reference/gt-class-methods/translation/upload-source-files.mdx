...
*Note: `uploadSourceFiles` requires an API key (including the deprecated `devApiKey` alias) or a [token provider](/docs/platform/core/reference/api-client#usertokenprovider), plus `projectId` on the GT instance.*
...
| `transformFormat` | Serialized from the shared `FileUpload` input, but not a supported source-upload endpoint field. Set the output format on the file reference passed to [`enqueueFiles`](/docs/platform/core/reference/gt-class-methods/translation/enqueue-files). | [`FileFormat`](/docs/platform/core/reference/types/file-format) | Yes |
...
| `branchId` | Branch to upload the file into. Uses the default branch when omitted. | `string` | Yes |
...
type UploadFilesResponse = {
  ...
  count: number; // wrapper-derived count of uploaded references
...
type FileReference = {
  fileId: string;
  versionId: string;
  branchId: string; // resolved branch ID
  fileName: string;
...
The response includes the resolved `branchId`, including when the request selects the default branch. It does not populate `transformFormat`; set that field before passing a reference to [`enqueueFiles`](/docs/platform/core/reference/gt-class-methods/translation/enqueue-files) when you need format conversion.
