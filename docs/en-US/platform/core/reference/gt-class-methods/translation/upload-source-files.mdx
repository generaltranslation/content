...
type FileReference = {
  fileId: string;
  versionId: string;
  branchId: string; // the requested branch or resolved default branch
  fileName: string;
...
The API always returns `branchId`, including when it selects the default branch. It does not populate `transformFormat`; set that field before passing a reference to [`enqueueFiles`](/docs/platform/core/reference/gt-class-methods/translation/enqueue-files) when you need format conversion.
