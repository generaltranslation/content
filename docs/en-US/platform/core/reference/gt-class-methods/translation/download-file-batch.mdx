...
- **Order.** Do not depend on response order. Match each result by its `fileId`, `branchId`, `versionId`, and `locale`.
...
- Do not depend on response order; match results by file, version, branch, and locale identifiers.
