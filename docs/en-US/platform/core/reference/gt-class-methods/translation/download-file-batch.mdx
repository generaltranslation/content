...
*Note: `downloadFileBatch` requires an API key (including the deprecated `devApiKey` alias) or a [token provider](/docs/platform/core/reference/api-client#usertokenprovider), plus `projectId` on the GT instance. Batches run concurrently by default.*
...
- **Locale resolution.** Requests use canonical service locales. An exact requested identity is restored when one unambiguous request matches; otherwise the response uses [configured-identity recovery](/docs/platform/core/reference/gt-class/constructor#how-it-works). Multiple indistinguishable aliases do not guarantee distinct response labels.
