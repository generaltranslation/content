...
*Note: every method on this page calls the General Translation API and requires a project ID and either an API key or a [token provider](/docs/platform/core/reference/api-client#usertokenprovider) on the [`GT`](/docs/platform/core/reference/gt-class/constructor) instance. Locale codes in arguments are normalized to their canonical form before the request.*

Management methods use the shared [retry and timeout rules](/docs/platform/core/reference/api-client#retrypolicy). Keep each method's documented timeout spelling; numeric zero is passed through to the transport.
...
Fetches metadata for the authenticated project: its name, Organization, default and current locales, and review settings. Response locales recover [configured identities](/docs/platform/core/reference/gt-class/constructor#how-it-works); the first matching identity wins when aliases are ambiguous.
...
Returns a `PublishFilesResult` with per-file `results` (`{ fileId, versionId, branchId, success, locale?, error? }`). A `locale` on a result indicates a translation; its absence indicates a source file. Returned locales recover [configured identities](/docs/platform/core/reference/gt-class/constructor#how-it-works), using the first matching identity when aliases are ambiguous.
...
Each `SubmitUserEditDiff` includes `fileName`, `locale`, `diff`, `branchId`, `versionId`, `fileId`, and `localContent`. The instance resolves custom locale aliases before submission, then the API resolves each locale to the supported locale used for storage. Equivalent codes such as `ja` and `ja-JP` update the same stored translation. Diffs are batched automatically. Resolves when submission succeeds.
