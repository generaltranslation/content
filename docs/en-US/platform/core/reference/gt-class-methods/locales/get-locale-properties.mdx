...
*Note: `name` and `nativeName` use dialect-style names (for example, `"Canadian French"` / `"français canadien"`), not the `"Language (Region)"` form shown in the source docs; use `nameWithRegionCode` for the region-suffixed form. Example outputs below reflect the current library behavior.*
