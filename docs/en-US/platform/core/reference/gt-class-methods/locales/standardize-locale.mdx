...
*Note: the source docs previously stated that invalid input returns an empty string and that underscores are converted to hyphens. In the current library code, `standardizeLocale` delegates to [`Intl.getCanonicalLocales`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/getCanonicalLocales): it normalizes casing on hyphenated codes and returns the input unchanged when the code cannot be standardized (underscore-separated codes such as `en_US` are not recognized and are returned as-is).*
