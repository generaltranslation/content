...
Call `isValidLocale` on a `GT` instance with an optional locale code. When omitted, it uses the instance's `targetLocale`.
...
- **Custom mapping bypass.** Locales present in a [`CustomMapping`](/docs/platform/core/reference/types/custom-mapping) always validate as `true`, bypassing standard validation.
