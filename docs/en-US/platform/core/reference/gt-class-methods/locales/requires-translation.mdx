...
Call `requiresTranslation` on a `GT` instance with optional source and target locales. When omitted, it uses the instance's `sourceLocale` and `targetLocale`.
...
*Note: the source docs stated that different dialects of the same language (for example, `en-US` → `en-GB`) do not require translation. In the current library code they do — only the exact same dialect short-circuits to `false`. The behavior below reflects the code; flag for human review against the targeted release.*
