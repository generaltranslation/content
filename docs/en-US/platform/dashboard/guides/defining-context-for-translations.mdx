...
However, for major changes, you can also use **Export** to download a group's Glossary and Custom Prompts. **Import** accepts CSV, TSV, JSON, and text files. It fills empty fields only: existing Custom Prompts, definitions, and translations are never overwritten, but an import can add missing translations to an existing term.
...
Review the **Import Summary** before confirming. It lists the Custom Prompts, terms, definitions, and translations that will be added. Unsupported locales are skipped and listed in the preview.
