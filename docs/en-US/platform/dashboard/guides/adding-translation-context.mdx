...
Custom Prompts can be global or locale-specific. Use locale-specific Custom Prompts when guidance should apply only to one target language or region.
...
4. Add relevant **Glossary** terms and **Custom Prompts**.
...
However, for major changes, you can also use **Export** to download a group's Glossary and Custom Prompts. Then use **Import** to fill an empty Glossary and Custom Prompts from a supported file.
