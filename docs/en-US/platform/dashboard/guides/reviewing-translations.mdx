...
- **Files** for Markdown, JSON, gettext, Lottie, SVG, and other file-based content.
...
These formats can also be edited as raw content. For keyed JSON, YAML, and `.strings` files, toggle **Raw**; SVG files open in the raw editor directly and must remain well-formed XML. The Raw view is read-only for PO, POT, and Android files so edits cannot break their structure. Use **Search in file** to find text within the open file.
...
SVG downloads are UTF-8 XML documents with translated text and completed layout adjustments.
