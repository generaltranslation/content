...
- HTML translations open in the structured editor and show one row per translatable text run and for the `alt`, `title`, `placeholder`, `label`, `aria-label`, and `aria-description` attributes. Inline markup stays inside its text run, while `script`, `style`, and `template` content is omitted.

Use **Raw** to inspect the underlying content. The Raw view is editable for JSON, YAML, `.strings`, and `.xcstrings` translations, and read-only for HTML, PO, POT, and Android files. Saving an `.xcstrings` edit updates only the selected target locale in the catalog. SVG files open in the raw editor directly and must remain well-formed XML. Use **Search in file** to find text within the open file.
