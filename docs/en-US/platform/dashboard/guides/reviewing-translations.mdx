...
For JSON, YAML, and `.strings` files, toggle **Raw** to edit the underlying file content in a code editor. The Raw view is read-only for PO, POT, and Android files so edits cannot break their structure. Use **Search in file** to find text within the open file.
