...
- **Files** for Markdown, JSON, gettext, Lottie, and other file-based content.
...
Apple `.strings` and `.stringsdict` files and Android `strings.xml` files open in the raw editor directly. The platform validates the complete document before applying an edit.
