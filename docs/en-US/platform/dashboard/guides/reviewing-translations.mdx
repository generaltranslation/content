...
Use **Raw** to inspect the underlying content. The Raw view is editable for JSON, YAML, and `.strings` translations, and read-only for PO, POT, and Android files. SVG files open in the raw editor directly and must remain well-formed XML. Use **Search in file** to find text within the open file.
