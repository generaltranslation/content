...
You pay per workflow: cost is based on the number of input tokens, workflow type (build time, runtime, or development), file format, and any context you provide. Google Slides also has a separate layout-fidelity charge for each completed slide.
...
Translation is charged per token of input at a base rate set by file format and workflow type. All token rates are per 10,000 input tokens; the Google Slides build-time rate also includes a per-slide charge:

| Format | Build-time | Runtime | Development |
| --- | --- | --- | --- |
| GT | $20 | $1 | $4 |
| MDX / Markdown | $10 | — | — |
| Google Slides | $10 + $0.50 per completed slide | — | — |
| JSON | $10 | — | — |
| YAML | $10 | — | — |
...
- **Context** — for each 500 context tokens, providing extra context adds **$0.10 per 10,000 input tokens**, set per Project.
