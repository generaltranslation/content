...
- **Starter — $0/month.** For individuals and small teams. There is no monthly fee; you pay only for what you use. Includes unlimited users and languages, up to 64 Projects, the [Translation Editor](/docs/platform/dashboard/guides/reviewing-translations), GitHub integration, and the [Locadex](/docs/platform/locadex/quickstart) AI agent.
- **Enterprise — custom.** For larger teams with complex localization needs. Everything in Starter, plus forward-deployed localization engineers, custom workflows for any file format or framework, shared context that can be assigned across Projects, and enterprise-grade security, SLAs, and support.
...
- **Platform Context** adds a surcharge based on input tokens and the exact amount of context provided. Context usage is proportional and is not rounded into token blocks.
...
Upgrading removes your rate limit, raises the Project limit from 1 to 64, and unlocks features including Locadex, team invites, auto-reload, the Translation Editor, and version branching.
