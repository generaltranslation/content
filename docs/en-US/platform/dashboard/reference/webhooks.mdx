...
Each attempt shows a status, endpoint URL, and timestamp. Expand a completed attempt to see whether it was **Automatic** or **Manual**, its round-trip latency, and an **Error** summary for transport failures such as connection errors or timeouts. Pending attempts without a recorded outcome cannot be expanded.
