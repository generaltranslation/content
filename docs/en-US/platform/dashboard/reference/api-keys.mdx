...
Set **Project API keys** to **Write** for automation that calls [Create a project API Key](/docs/platform/openapi/reference/project/create-api-key). The endpoint accepts a name and an optional nonempty `permissions` list. Omit the list to delegate all grantable project permissions held by the request identity, or provide it to create a more limited key. Every selected permission must be held by the request identity; unavailable selections are rejected. The optional `type` field is deprecated and ignored. The request identity can be an Organization key or an OAuth user access token; project keys cannot hold the required `project:api_keys:write` permission.
