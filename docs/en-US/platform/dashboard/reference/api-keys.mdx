...
Set **Project API keys** to **Write** for automation that calls [Create a project API Key](/docs/platform/openapi/reference/project/create-api-key). The request identity can be an Organization key targeting its own Organization's project or an OAuth user access token with access to the project. Project keys cannot hold the required `project:api_keys:write` permission and cannot create other keys.

The HTTP endpoint accepts a required `name` (trimmed, 1–255 characters) and an optional, nonempty `permissions` array of canonical project permission strings. Duplicates are ignored. Invalid selections—including unknown permissions, Organization permissions, `project:api_keys:write`, `gt:*`, or an empty array—return `400`. If any valid selection is unavailable to the caller, the request returns `403` without creating a partial key. OAuth callers need both the creation scope and each delegated scope, together with the corresponding current membership permissions.
