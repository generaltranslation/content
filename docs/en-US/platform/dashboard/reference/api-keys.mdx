...
Set **Project API keys** to **Write** for automation that calls [Create a project API Key](/docs/platform/openapi/reference/project/create-api-key). The endpoint accepts a name and creates a project key with the project permissions held by the request identity; it does not accept custom permissions. Its optional `type` field is deprecated and ignored. The request identity can be an Organization key or an OAuth user access token; project keys cannot hold the required `project:api_keys:write` permission.
