...
Both scopes support **All** or **Custom** permissions. **All** grants every permission you can delegate within that scope. **Custom** lets you select a smaller set; select at least one permission. You can only grant permissions you hold.
...
Set **Project API keys** to **Write** for automation that calls [Create a project API Key](/docs/platform/openapi/reference/project/create-api-key). The endpoint accepts a name and an optional permission list. Omit the list to grant every project permission the request identity can delegate, or include one or more permissions the request identity holds to create a narrower key. Duplicate permissions are ignored, and unavailable permissions are rejected. The optional `type` field is deprecated and ignored. The request identity can be an Organization key or an OAuth user access token; project keys cannot hold the required `project:api_keys:write` permission.
