...
| **Editor**    | Can manage project files, queue file translations, and read, edit, and review translations. No access to settings, members, or billing. |
...
| Create projects (`org:projects:create`) |   ✓   |           |        |        |
| Manage members           |   ✓   |           |        |        |
| View usage               |   ✓   |     ✓     |        |        |
...
The public API uses `org:projects:create` to create projects and to discover create-capable Organizations with an OAuth user token. This is distinct from `org:manage`; only **Admin** has it among the default roles.
...
| Read project files       |   ✓   |     ✓     |   ✓    |        |
| Write project files      |   ✓   |     ✓     |   ✓    |        |
| Generate runtime translations |   ✓   |     ✓     |        |        |
| Queue file translations |   ✓   |     ✓     |   ✓    |        |
| Read Locadex automations and settings  |   ✓   |     ✓     |        |        |
| Write Locadex automations and settings |   ✓   |     ✓     |        |        |

Project and Organization API keys are separate permissions: the **Developer** role can create project API keys but not Organization API keys. Key creators can delegate only permissions they hold; OAuth callers are also limited by the token's scopes. Webhook permissions are Organization-scoped, and webhooks require a Team or Enterprise plan; see the [webhooks reference](/docs/platform/dashboard/reference/webhooks).

The [Google Drive MCP tools](/docs/integrations/google-drive/reference/mcp-tools) require **Read project files** for discovery and status, and both **Write project files** and **Queue file translations** to start translation. The default **Admin**, **Developer**, and **Editor** roles have those permissions.
