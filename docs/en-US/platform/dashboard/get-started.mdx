...
<Accordions className="faq-accordions">
  ...
  <Accordion title="How do I get an API key?">
    Create a project, then open **Project > API Keys** or **Organization > Developer > API Keys**. Project keys are limited to one project, while Organization keys can work across projects. Both scopes support **All** or **Custom** permissions. (See [API keys](/docs/platform/dashboard/reference/api-keys)).
  </Accordion>
