...
<Accordions className="faq-accordions">
  ...
  <Accordion title="How do I get an API key?">
    Create a project, then open **Project > API Keys** or **Organization > Developer > API Keys**. Project keys are used for one project, and Organization keys for automation across Projects. Both support custom permissions. (See [API keys](/docs/platform/dashboard/reference/api-keys)).
  </Accordion>
