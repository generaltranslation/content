...
Each automation can run manually, when a pull request changes, or when a commit is pushed. Pull request triggers and `/locadex` comments apply only to branches in the linked repository; pull requests from forks are ignored because Locadex cannot access or push to the fork.
