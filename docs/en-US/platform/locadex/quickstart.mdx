...
Locadex supports automated setup for Next.js App Router, Next.js Pages Router, Mintlify, Fern, and Docusaurus Projects. Vite, Gatsby, React, Redwood, React Router, and TanStack Start are supported automation targets but require manual CLI setup first.
...
5. **Run setup.** Open the automation and click **Run setup**. Choose the source locale and target languages. For a framework with automated setup, click **Install** so Locadex prepares the repository and opens a setup pull request. For another React framework, run `npx gt@latest` in the target directory, then click **Skip Setup**.
6. **Review and merge.** If Locadex opened a setup pull request, open it from the automation page, review the changes, and merge it before running the automation.
