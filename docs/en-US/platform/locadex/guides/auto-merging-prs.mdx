...
description: How to configure Locadex to merge pull requests automatically and handle GitHub rules which require review.
related:
  links:
...
Auto-merge eliminates manual PR review when GitHub permits the merge. When enabled, Locadex tries to squash-merge each generated pull request after publishing its changes.
...
2. Find every ruleset which prevents automatic merges. Common blockers include **Require a pull request before merging**, required status checks, and required review approvals.
3. Open each blocking ruleset, choose **Add bypass**, and select **Locadex Agent**.
4. Select **Save**.

If GitHub blocks an automatic merge, Locadex completes the automation, leaves the pull request open, and marks its check as **Action required**. Complete the required reviews and status checks, then merge the pull request in GitHub.
