...
description: How to configure Locadex to merge pull requests automatically and handle GitHub rules that block the merge.
related:
  links:
...
Auto-merge reduces manual PR review when Locadex can squash-merge without repair changes and GitHub accepts the merge. When enabled, Locadex tries to squash-merge each generated pull request after publishing its changes.
...
1. Open your GitHub repository **Settings** and select **Rules > Rulesets**.
2. Find every ruleset that prevents automatic merges. Common blockers include **Require a pull request before merging**, required status checks, and required review approvals.
3. Open each blocking ruleset, choose **Add bypass**, and select **Locadex Agent**.
4. Select **Save**.

If GitHub blocks an automatic merge, Locadex completes the automation, leaves the pull request open, and marks its check as **Action required**. The same status appears when repair changes require review or auto-merge is off. Complete the required reviews and status checks, then merge the pull request in GitHub.
...
- Your repository does not permit Locadex to bypass rules that block an immediate squash merge.
