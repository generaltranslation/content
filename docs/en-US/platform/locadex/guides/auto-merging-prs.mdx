...
2. Find each ruleset which prevents automatic merges. Common blockers include the "require a pull request before merging," rule, required status checks, and required review approvals.
3. Select the blocking rule, choose **Add bypass**, and select **Locadex Agent**.
4. Select **Save**.

If GitHub blocks an automatic merge, the automation completes, but Locadex leaves the pull request open and marks its check as **Action required**. Complete the required reviews and status checks, then merge the pull request in GitHub.
