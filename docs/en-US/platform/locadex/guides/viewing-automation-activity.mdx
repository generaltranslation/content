...
- `WORKFLOW_TIMEOUT` and `WORKFLOW_CANCELLED` indicate that the run did not reach completion.
- `WORKFLOW_ABORTED` indicates that the run was aborted before finishing. Billing blocks stop the run before work begins and use this code for an exhausted balance, billing-period spend limit, failed payment, or ended subscription. Other internal aborts may also use this code. Follow the run message and update **Organization > Settings > Billing** when billing is blocked. (See [Managing billing](/docs/platform/dashboard/guides/managing-billing)).
