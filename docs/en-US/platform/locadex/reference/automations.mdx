...
An automation cannot produce correct results until its target directory is set up. If setup has not completed, the detail page shows **Run setup**.

Choose the source locale and target languages, then start setup. Next.js App Router, Next.js Pages Router, Mintlify, Fern, and Docusaurus targets support automated setup: click **Install**, then merge the setup pull request before running the automation.

For Vite, Gatsby, React, Redwood, React Router, and TanStack Start targets, run `npx gt@latest` in the target directory, then click **Skip Setup** to enable Locadex. These frameworks support Locadex automations after manual CLI setup.
