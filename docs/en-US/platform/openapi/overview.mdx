...
Every endpoint requires a project or Organization API key in the standard `Authorization` bearer header. The endpoint reference lists any permission required by each operation.
...
Discovery endpoints do not require a project ID. [`GET /v2/orgs`](/docs/platform/openapi/reference/project/list-orgs) returns the Organization bound to the API key. [`GET /v2/projects`](/docs/platform/openapi/reference/project/list-projects) returns projects accessible to a key with `project:files:read`.
...
- `project:files:read` for listing projects, downloading files, and reading file info, translation status, branch info, orphaned files, project info, and job info.
