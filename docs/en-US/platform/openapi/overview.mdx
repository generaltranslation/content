...
Use the correct key scope:

- **Project** with prefix `gtx-api-`: bound to one project and usable in any environment
...
The authorization server reserves the `gt:*` wildcard scope for registered first-party clients. Dynamically registered applications must request individual permission scopes and cannot request `gt:*`.

### Permissions

Each endpoint requires a permission on the request identity. Organization and project keys created in the Dashboard can use **All** grantable permissions or a **Custom** set. Project keys created through `POST /v2/projects/{projectId}/api-keys` receive the default project permission bundle, limited to permissions the request identity can delegate. OAuth user access tokens must include the matching scope, and the signed-in user must still hold that permission.
...
- **After authentication:** each route group limits requests by authenticated actor, including API keys, OAuth users, and Locadex sessions. Limited routes without an authenticated actor use the client IP. Related operations can share a route-group counter.

Post-authentication limits vary by operation:
