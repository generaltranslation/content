...
Most endpoints require an API key as a bearer token in the standard `Authorization` header. The CLI wizard session endpoints are unauthenticated.
...
- **CLI wizard:** 60 requests/minute for creating, polling, and deleting sign-in sessions.
