...
- Use `https://api2.gtx.dev` for project and file endpoints.
- Use `https://runtime2.gtx.dev` for runtime translation, including `POST /v2/translate`.
...
- **Organization** with prefix `gtx-org-`: works across Projects. When using an Organization key on Project-scoped endpoints, you must send `x-gt-project-id`. Organization-scoped endpoints, such as `POST /v2/projects`, derive the Organization from the key.
