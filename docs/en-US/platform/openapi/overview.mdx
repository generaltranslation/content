...
For OAuth access, request the scopes your application needs. You must also have permission to perform each operation. Creating an API key requires both key-creation access and permission to grant every selected permission.
...
- [`GET /v2/projects`](/docs/platform/openapi/reference/project/list-projects) lists projects you can access. Requires `project:files:read`. Each result contains `id`, `name`, `orgId`, and `orgName`.
- [`GET /v2/orgs`](/docs/platform/openapi/reference/project/list-orgs) lists Organizations where you can create projects. Requires an OAuth user token with `org:projects:create`. Each result contains `id` and `name`. Missing scope returns `403`; no eligible Organizations returns an empty page.

Both endpoints return results in ascending ID order, with a `projects` or `orgs` array and `nextCursor`. Use optional `limit` (integer 1–100, default 50), then pass a non-null `nextCursor` unchanged as `cursor` to fetch the next page; `null` ends pagination. Neither endpoint requires `gt-project-id` or supports an `orgId` query filter.
...
Project and Organization discovery and project/key creation share a request limit. Refreshing an OAuth token does not reset your rate limit. Authentication attempts may also be rate-limited.
