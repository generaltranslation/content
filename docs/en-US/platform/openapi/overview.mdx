...
- **Organization** with prefix `gtx-org-`: works across Projects. When using an Organization key on Project-scoped endpoints, send `x-gt-project-id`. Organization-scoped endpoints, such as `POST /v2/projects`, derive the Organization from the key.
