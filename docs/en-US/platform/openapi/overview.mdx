...
Each endpoint requires a permission on the request identity. Both Organization and project keys support **All** or **Custom** permissions in the Dashboard, limited to permissions the creator can grant. When you create a key through the project API-key endpoint, omit `permissions` to grant every project permission the request identity can delegate or provide a narrower permission list. OAuth user access tokens must include the matching scope, and the signed-in user must still hold that permission.
