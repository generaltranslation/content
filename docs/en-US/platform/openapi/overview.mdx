...
- **Organization** with prefix `gtx-org-`: works across Projects. Most Project-scoped endpoints require `gt-project-id`. Endpoints with a Project ID in the path, such as [`POST /v2/projects/{projectId}/api-keys`](/docs/platform/openapi/reference/project/create-api-key), use that path parameter instead. Organization-scoped endpoints, such as `POST /v2/projects`, derive the Organization from the key.
