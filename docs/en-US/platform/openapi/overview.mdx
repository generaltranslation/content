...
For `POST /v2/translate`, `maxChars` constrains generation only when it is a positive integer. Other values are ignored.
...
- **Default:** 200 requests/minute for Project creation, branches, tags, Project info, job info, file info, translation status, and runtime translation.
...
<Callout type="warn">
  The schema accepts `SVG` as a file format, but generated SVG translations are not available in the current translation pipeline. Do not enqueue SVG files for translation.
</Callout>