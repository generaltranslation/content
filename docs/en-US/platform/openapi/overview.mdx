...
After sign-in, [`GET /v2/orgs`](/docs/platform/openapi/reference/project/list-orgs) lists the Organizations where the user can create projects. [`GET /v2/projects`](/docs/platform/openapi/reference/project/list-projects) lists the projects the request identity can read. Both endpoints are cursor-paginated.

### Permissions

Each endpoint requires a permission on the request identity. Both Organization and project keys support **All** or **Custom** permissions in the Dashboard, limited to permissions the creator can grant. When creating a key through the project API-key endpoint, select a nonempty list of project permissions or omit the list to delegate every grantable project permission held by the request identity. OAuth user access tokens must include the matching scope, and the signed-in user must still hold that permission.
