...
Use the key scope that fits:

- **Project** with prefix `gtx-api-`: bound to one project and valid in every environment. Permissions are set per key.
...
Each endpoint requires a permission on the request identity. Organization keys and Project keys created in the Dashboard carry the permissions chosen at creation. Keys created through the Organization API are limited to the permissions the request identity can delegate. OAuth user access tokens must include the matching scope, and the signed-in user must still hold that permission.
