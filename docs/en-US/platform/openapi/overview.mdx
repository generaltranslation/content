...
<Callout type="warn">
  The hosted API creates projects at `POST /v2/orgs/{orgId}/projects`. `@generaltranslation/api` 0.2.0, `generaltranslation` 9.3.0, and `gt` 2.21.0 still call the removed `POST /v2/projects` route. Use the [Create a project](/docs/platform/openapi/reference/project/create-project) endpoint directly until compatible package releases are available.
</Callout>
