...
Managing webhook endpoints requires the `org:webhooks:write` permission. Viewing delivery history requires `org:webhooks:read`. By default, the **Admin** role has both permissions. See [roles and permissions](/docs/platform/orgs/roles) for details.
