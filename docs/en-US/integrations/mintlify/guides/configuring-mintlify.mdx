...
New translation automations default to **A pull request changes**, with **Auto-merge** off. Choose **A commit is pushed** to translate changes after they reach the target branch. Turn on **Auto-merge** only after allowing Locadex to bypass rules that would block the immediate squash merge; repair changes still require review. (See [Auto-merging Locadex PRs](/docs/platform/locadex/guides/auto-merging-prs)).
