...
When updating an existing `gt.config.json`, Locadex changes the locale settings but preserves the current [`files`](/docs/integrations/mintlify/reference/config#files) rules and values already set under [`options`](/docs/integrations/mintlify/reference/config#options). It adds detected OpenAPI specifications and fills in Mintlify defaults only when they are missing.
