...
Locadex watches your default branch. When you push new or changed pages, it translates the updates and opens or updates a pull request with the changes. It reuses an open PR for the same workflow branch and opens a new one after the previous PR is merged or closed. For documentation repositories, these pull requests can merge automatically once checks pass, so your translated docs stay current without manual steps. Review or change this behavior under [Auto-merge](/docs/platform/locadex/guides/auto-merging-prs).
