...
description: Find the files and content translated in a Mintlify project. Reference for Mintlify supported content.
...
`docs.json` is localized with a Mintlify preset: group, tab, item, anchor, dropdown, product, description, header, and label text is translated. Navigation root pages and page paths are localized, and the navigation is restructured into Mintlify's multi-language layout with one entry per language.

In show-default-locale mode, the preset also localizes the source and destination paths of existing top-level redirects. The integration does not create new redirect entries.

### OpenAPI specifications

Locadex detects OpenAPI 3.x specifications stored in `.json`, `.yaml`, or `.yml` files. It translates `summary` and `description` fields, writes each translated specification under its locale directory, and registers the localized files with Mintlify.
