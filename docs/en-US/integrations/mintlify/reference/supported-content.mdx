...
`docs.json` is localized with a Mintlify preset: navigation labels are translated, and the navigation is restructured into Mintlify's multi-language layout with one entry per language.
