...
Turn on **Auto-refresh** to poll translation status, or use **Refresh Status** to check once.
...
Use **Patch Document References** on the Translations tool to run this across existing translated documents. In the single-document dialog, it also runs automatically after import (auto-patch is on by default there).
