...
If a document-level translation was deleted while its `translation.metadata` entry remained, the next import creates a replacement translation and updates the metadata reference. Import still stops when the source document itself no longer exists.
