...
Use **Patch References** on the Translations tool to run this across existing translated documents. In the single-document dialog, auto-patch is off by default, so enable it there to run patching automatically after each import.
