...
- **Production project or Organization API key:** the project must use an Organization shared drive connection. A personal Google account cannot supply Drive access for an API-key caller.
- **Locadex:** the project must use an Organization shared drive connection.

The Organization needs the project integrations feature. Organization shared drive access also requires its separate Enterprise feature. Configure both connection modes in the [Connection reference](/docs/integrations/google-drive/reference/connection).

MCP and API access use project file and translation-queue permissions. Dashboard controls use the separate `translations:content:write` permission.
