...
- **Project API key:** the key must include the permissions required by the tool, and the project must use an Organization shared drive connection.
...
| `targetLocales` | One to 100 supported locale codes allowed by the project's locale whitelist. | `string[]` | No | — |
...
The tool removes a target code that exactly matches the resolved source. The request fails before enqueueing if no target remains or if a target represents the same dialect with a broader or narrower code, such as `en` and `en-GB`. To translate between variants, use two specific codes that distinguish them, such as `en-US` and `en-GB`.
...
Locale validation errors fail the request before it returns this result. Failures that occur while starting individual locales appear in `failed` and `errors`, while other locales can still be queued.
