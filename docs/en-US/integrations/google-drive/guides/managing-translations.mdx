...
General Translation reads the latest source and the existing target copy. It reuses matching translations, translates new or changed text, and applies the result to that copy.
...
Force retranslation starts from the source again, overwrites existing translations in the target copy, and incurs additional usage. The warning opens when you click **Retranslate**, not when you select the checkbox. You can suppress it for the rest of the current browser session.
