...
The token is sent as the `Authorization` header on every Management API request the integration makes. It must have access to the space's stories and components, because the integration reads stories, reads and updates story content, and (for [Auto-enable supported fields](/docs/integrations/storyblok/guides/translate-content#auto-enable)) reads and updates component schemas.
