...
Without a supported plan, the Dashboard shows an upgrade prompt in place of the integrations catalog, so the **Connect** action is not reachable. Without `project:write`, the **Connect** action is unavailable. The same plan requirement applies when running translations; the connect and translation actions reject requests from unsupported plans.
