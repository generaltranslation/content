...
| `sourceLocale` | Identifies the locale visible in the recording. | `string` | Yes | Named locale cookie, then `locales[0]` |
...
The cookie fallback applies only when `localeCookieName` is set. The package does not assume a framework-specific cookie name.
