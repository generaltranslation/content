...
Import replayer values from `gt-rrweb/replay`. This entry point exports the React wrapper and requires React and React DOM 18 or later, even when you use only `createGTReplayer`; the DOM player also requires the optional `@rrweb/replay` peer dependency.
