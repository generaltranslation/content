...
- React and React DOM 18 or later. The shared replay entry also requires these peers when you use only its DOM API.
...
The type assertion describes the JSON file produced by the recorder; it does not validate an arbitrary uploaded file. Give unframed recordings an explicit playback height, as in the example.
