...
related:
  links:
    ...
    - /docs/integrations/rrweb/reference/recorder

---

The `gt-rrweb` integration records a browser walkthrough once with [rrweb](https://www.rrweb.io/) and replays the same session in each of your locales, using the translations your app already publishes.

Recording happens inside your React app: the recorder captures the rrweb event stream, matches the rendered text to your translation catalogs, and saves everything as one bundle. Replay works anywhere in the browser, as a React component or a framework-agnostic function, and lets viewers switch locales without re-recording.
...
- A browser application that renders translations with [`<T>`](/docs/react/reference/components/t) and publishes its translation catalogs.
...
Click **Record**, complete the flow in the source locale, then click **Stop**. Clicks, navigation, DOM changes, scroll position, and page content are captured. Form input values are masked and mouse-move telemetry is left out.
