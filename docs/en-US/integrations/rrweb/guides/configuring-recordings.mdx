...
The loader can read local files, fetch a content delivery network (CDN), or use the same source as your application. Set `sourceLocale` to the locale visible during capture. If you omit it, harvesting reads the cookie named by `localeCookieName`, when configured, and then falls back to the first locale passed to `start()`.
