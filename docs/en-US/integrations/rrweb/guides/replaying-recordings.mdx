...
The player renders the recording with playback controls, a click-based cursor, scrubbing, light and dark themes, full-screen playback, and a locale switcher when the bundle carries more than one locale. Use the React component in React views or the DOM function when you manage the player lifecycle yourself; both come from `gt-rrweb/replay`.
