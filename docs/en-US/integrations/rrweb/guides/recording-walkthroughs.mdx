...
related:
  links:
    ...
    - /docs/integrations/rrweb/reference/harvest
...
Content rendered by [`<T>`](/docs/react/reference/components/t) is matched through the DOM hashes that [`_tagIds`](/docs/react/reference/config#tag-ids) enables. To also translate bare `gt()` or `useGT()` strings, pass a `hashMessage` function; interpolated strings whose rendered value differs from their source template stay in the source locale. See [Harvest options](/docs/integrations/rrweb/reference/harvest#harvest-locales).
