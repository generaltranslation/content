...
```jsx
<T>Last updated: <DateTime>{date}</DateTime></T>
```

Would be represented in GT JSX as:

```ts
["Last updated: ", { k: "date", v: "d", i: 1 }]
```

#### Example 4: RelativeTime

```jsx
<T>Published <RelativeTime>{date}</RelativeTime></T>
```

Would be represented in GT JSX as:

```ts
["Published ", { k: "date", v: "rt", i: 1 }]
```

#### Example 5: With name prop
