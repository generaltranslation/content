...
Install the `gt-react-native` and `gt` packages:
...
<Callout type="warn">
  **Dev vs prod keys:** Use a `gtx-dev-` key locally. Use a `gtx-api-` key in CI/CD if you run `gt translate` during deploy.
...
<Accordions>
  ...
  <Accordion title="You are using invalid locale codes in your configuration.">
    ```
    @generaltranslation/react-core: Error: You are using invalid locale codes in your configuration.
    You must either specify a list of valid locales or use "customMapping" to specify aliases for the following invalid locales: en, fr, zh. 
    ```
    This error typically occurs when locales are not being properly polyfilled. Make sure that you have either polyfilled the locales manually or are using the babel plugin correctly.

    If you make updates to the babel plugin, you need to clear the cache to make sure the changes are applied.
