...
Initialize the library in your entry file with `initializeGT`, then wrap your app with the provider.
...
initializeGT({
  ...
  devApiKey: process.env.EXPO_PUBLIC_GT_API_KEY,
});
...
import { T, useLocaleSelector, useMessages } from 'gt-react-native';
      ...
      <View style={styles.nav}>
        {locales.map((l) => (
          <Pressable key={l} onPress={() => setLocale(l)}>
            <Text style={[styles.navItem, l === locale && styles.activeLocale]}>
              {l}
            </Text>
          </Pressable>
        ))}
...
<Callout type='warn'>
  **Note:** Expo Go does not work with `gt-react-native` because it includes a native module. Use a [development build](https://docs.expo.dev/develop/development-builds/introduction/) instead.
</Callout>
...
- Switching languages with [`useLocaleSelector`](/docs/react-native/api/helpers/use-locale-selector)
