...
<Accordions>
  ...
  ```
    ...
    This can also be caused by using an expo build.
    Make sure to switch to a development build by running `npx expo start --dev-client` or `npx expo run:ios`/`npx expo run:android`.
  </Accordion>
