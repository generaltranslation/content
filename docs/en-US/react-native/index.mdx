...
<Callout type="warn">
  **Dev vs prod keys:** Use a `gtx-dev-` key locally. Use a production key (`gtx-api-`) in CI/CD if you run `gt translate` during deploy.
...
export default function RootLayout() {
  return (
    <GTProvider
      ...
      devApiKey={process.env.EXPO_PUBLIC_GT_DEV_API_KEY} // only needed for development
    >
      {children}
    </GTProvider>
  )
