...
`gt-rrweb` records a browser walkthrough with rrweb, maps the captured text to your existing translation catalogs, and plays the same session in each locale. The replayer includes a React component and a function that can be called from another browser framework, but the package still depends on `react` and `react-dom`.
...
<Tabs items={['npm', 'yarn', 'bun', 'pnpm']}>
  <Tab value="npm">
    ```bash
    npm install gt-rrweb @rrweb/record @rrweb/replay @rrweb/types react react-dom
    ```
  </Tab>

  <Tab value="yarn">
    ```bash
    yarn add gt-rrweb @rrweb/record @rrweb/replay @rrweb/types react react-dom
    ```
  </Tab>

  <Tab value="bun">
    ```bash
    bun add gt-rrweb @rrweb/record @rrweb/replay @rrweb/types react react-dom
    ```
  </Tab>

  <Tab value="pnpm">
    ```bash
    pnpm add gt-rrweb @rrweb/record @rrweb/replay @rrweb/types react react-dom
    ```
  </Tab>
</Tabs>
...
Mount [`GTRecorder`](/docs/rrweb/reference/recorder#gt-recorder) once near the application root. Pass the same catalog loader your app uses, then call [`useRecorder()`](/docs/rrweb/reference/recorder#use-recorder) from any control component.
...
The player starts in the recorded source locale and shows controls for the locales listed in the bundle.
