...
title: Quickstart
description: Add multiple languages to a Vue application in under 10 minutes.
related:
  links:
...
- Node.js `^20.19.0 || >=22.12.0` for the current Vite toolchain

<Callout type="info">
  **0.x package:** `gt-vue` is unstable. Its API and behavior may change between releases independently of the stable React packages.
</Callout>
...
Sign in with [`gt login`](/docs/cli/reference/commands/login):

```bash
npx gt login
```

Login does not select a project or write environment files. Set your existing project's ID in the environment, or add [`projectId`](/docs/cli/reference/config#project-id) to the config from Step 2:

```bash title=".env.local"
GT_PROJECT_ID="your-project-id"
```

For guided project selection or creation, use the separate [`gt init`](/docs/cli/reference/commands/init) flow; do not rerun the setup wizard merely to authenticate this manually configured app.

<Callout type="warn">
  Keep `.env.local` out of version control. For CI, use a separately scoped `GT_API_KEY` from your secret store. Explicit tooling keys override login, including keys loaded from env files; never expose them in browser code. (See [CLI credentials](/docs/cli/guides/configuring#credentials)).
</Callout>
...
Open the local URL shown by your development server. Choose `es`, `fr`, or `ja` from the language selector and confirm that the heading and search placeholder display translated text.
