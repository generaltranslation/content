...
| [`maxChars`](#maxchars) | Requested maximum length for the translation. | `number` | Yes | — |
...
Maximum character count requested from translation tooling. This is translation metadata; the Vue runtime does not truncate rendered output.
...
Static `placeholder`, `title`, `alt`, `aria-label`, `aria-labelledby`, and `aria-describedby` props on HTML elements and custom components participate in the translation. Dynamic versions of those text props are not supported inside `<T>`.
