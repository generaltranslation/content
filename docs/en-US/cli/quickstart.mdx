...
It works with [`gt-react`, `gt-next`, and `gt-react-native`](/docs/react/overview), with [`gt-vue`](/docs/vue/quickstart), with third-party i18n libraries like `next-intl` and `i18next`, and with standalone files such as JSON, YAML, Markdown, and MDX.
...
Write the file yourself and commit it so the CLI knows what to translate. A minimal config sets the source and target locales and a `files` entry so [`translate`](/docs/cli/reference/commands/translate) has something to work on. The `gt` entry below stores framework translations (from `gt-next`, `gt-react`, `gt-react-native`, or `gt-vue`) locally at the given path.
