...
| `--tag <value>` | Tag this translation run for version tracking (use `"git"` to auto-resolve from current commit) | `string` | |
...
Pass `"git"` as the tag value to automatically use the current git commit hash as the tag id and the commit message as the tag message:

```bash
npx gt translate --tag git
```

You can override the commit message with a custom one:

```bash
npx gt translate --tag git -m "Release 2.1 translations"
```
