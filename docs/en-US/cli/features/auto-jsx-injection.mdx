...
If your app uses an older `gt-react` major, also set [`legacyGtReactImportSource`](/docs/cli/reference/config#parsingflags) to `true` in `parsingFlags` so the compiler emits its injected imports from `gt-react/browser` instead of `gt-react`.
