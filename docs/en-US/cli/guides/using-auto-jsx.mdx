...
The compiler loads the root `gt.config.json` automatically. Automatic injection does not replace the [`gt-react` server-rendered setup](/docs/react/guides/configuring): initialize the runtime and provide the active locale and translations before rendering.
