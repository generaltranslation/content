...
Run [`gt init`](/docs/cli/reference/commands/init) to detect your framework, install dependencies, create the config file, and configure project access when needed.
...
## Authenticate the CLI [#credentials]

For interactive work, sign in before running API-backed commands. [`gt init`](/docs/cli/reference/commands/init) does this when its selected setup needs API access, or you can sign in directly:

```bash
npx gt login
```

The CLI uses an explicit API key when one is set and otherwise uses your stored login. Both methods require a project ID from `gt.config.json`, `GT_PROJECT_ID`, or `--project-id`.

For CI and other non-interactive environments, set a project key and project ID in secret environment variables. For a [custom key](/docs/platform/dashboard/reference/api-keys#create-project-keys), grant **Files > Write** and **Translation queue > Enabled**; add **Context > Write** when generating context.
...
Never store an API key in `gt.config.json`.
