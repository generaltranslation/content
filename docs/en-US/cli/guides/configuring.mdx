...
The [`translate`](/docs/cli/reference/commands/translate) command needs a project API key and project ID. Keep them out of `gt.config.json` and set them as environment variables instead. For a [custom key](/docs/platform/dashboard/reference/api-keys#create-project-keys), grant **Files > Write** and **Translation queue > Enabled**; add **Context > Write** when generating context.
