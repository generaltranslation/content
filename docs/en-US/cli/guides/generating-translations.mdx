...
*Note: Run [`translate`](/docs/cli/reference/commands/translate) in your CI pipeline before you build for production. For on-demand local previews, configure your SDK with a project key restricted to runtime translation.*

## Before you start [#before]

- Configure your project with a `gt.config.json`. See [Configuring the CLI](/docs/cli/guides/configuring).
- Bind a project ID and [sign in or supply a tooling key](/docs/cli/guides/configuring#credentials) with permissions for the workflow. Use a scoped key from your CI secret store for automated builds.
