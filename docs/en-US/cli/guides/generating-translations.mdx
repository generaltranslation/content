...
*Note: Run [`translate`](/docs/cli/reference/commands/translate) in your CI pipeline before you build for production. During development, use a project key through your framework's development credential setting for on-demand translation instead.*

## Before you start [#before]

- Configure your project with a `gt.config.json`. See [Configuring the CLI](/docs/cli/guides/configuring).
- Set a project key as `GT_API_KEY` and set `GT_PROJECT_ID` as environment variables.
