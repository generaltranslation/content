...
The CLI translates every file matched in `gt.config.json`. If you use `gt-next`, `gt-react`, `gt-react-native`, or `gt-tanstack-start`, it also scans your source code for inline content — such as [`<T>`](/docs/react/reference/components/t) components and [`useGT`](/docs/react/reference/hooks/use-gt) calls — and includes your dictionary file. Translations are saved to your codebase, ready to commit.

If you use `next-intl` or `i18next`, the CLI detects the library from your `package.json` and translates your files while preserving that library's string syntax.
