...
The command compares your local files against the last downloaded versions, computes a diff for anything that changed, and submits it. It does not enqueue any new translations. `translate` and `stage` run this same step automatically before enqueuing, so you only need `save-local` on its own when you want to sync edits without translating. To skip the step, pass `--no-save-local` for a single run or set [`options.saveLocal`](/docs/cli/reference/config#save-local) to `false` in `gt.config.json`.
