...
description: Translate your project with the General Translation CLI and save the results to your codebase or the CDN.
...
*Note: Run `translate` in your CI pipeline before you build for production. It is not meant for development, where you use a development API key for on-demand translation instead.*

## Before you start [#before]

- Configure your project with a `gt.config.json`. See [Configure the CLI](/docs/cli/guides/configure).
...
The CLI translates every file matched in `gt.config.json`. If you use `gt-next`, `gt-react`, or `gt-react-native`, it also scans your source code for inline content — such as `<T>` components and `useGT` calls — and includes your dictionary file. Translations are saved to your codebase, ready to commit.
