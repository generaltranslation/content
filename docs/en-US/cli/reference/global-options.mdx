...
| [`-q, --quiet`](#quiet) | Suppress informational logs. | `boolean` | Yes | `false` |
...
Suppresses banners, progress spinners, and informational logs. Warnings and errors remain visible. This does not suppress command data: [`gt api-key create`](/docs/cli/reference/commands/api-key-create) still writes the new secret to stdout.
...
*Note: Quiet mode suppresses logs, not interaction. Prompts still run. For CI, use a committed config and an explicit secret key, not [`gt init`](/docs/cli/reference/commands/init) or [`gt login`](/docs/cli/reference/commands/login). Even login's `--no-browser` mode needs human approval.*
...
For [`gt api-key create`](/docs/cli/reference/commands/api-key-create), diagnostics go to stderr. The secret is raw stdout even with JSON logging and is never captured by `GT_LOG_FILE`.
