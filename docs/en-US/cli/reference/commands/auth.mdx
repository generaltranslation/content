...
When you do not pass `--key-type`, the command prompts you to choose which environment variables to write. Project API keys are not tied to an environment, so every option writes the same key.
...
2. Generates a project API key and project ID.
3. Creates `.env.local` (and adds it to `.gitignore`) if needed.
4. Writes `GT_PROJECT_ID` and the key to `.env.local`, as `GT_API_KEY`, `GT_DEV_API_KEY`, or both depending on the key type.
...
| `-t, --key-type <type>` | Variables to write: `production` writes `GT_API_KEY`, `development` writes `GT_DEV_API_KEY`, and `all` writes both. | `string` | Yes | Prompted |
...
# Write only GT_API_KEY without the prompt
