...
- [`gt translate`](/docs/cli/reference/commands/translate) and [`gt stage`](/docs/cli/reference/commands/stage) run this same step automatically before enqueuing. Disable it for one run with `--no-save-local`, or for every run by setting [`options.saveLocal`](/docs/cli/reference/config#save-local) to `false` in `gt.config.json`.
