...
*Note: Requires a project ID and either a stored [`gt login`](/docs/cli/reference/commands/login) or a project key in `GT_API_KEY`.*
...
- An explicit API key takes precedence over the stored user login. Never store an API key in `gt.config.json`.
