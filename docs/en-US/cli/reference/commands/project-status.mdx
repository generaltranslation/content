...
1. Uses an explicit API key when set and otherwise uses the stored [`gt login`](/docs/cli/reference/commands/login). It resolves the project ID from flags, config, or environment variables.
...
The command exits with status 1 when authentication or the project ID is missing, or when the API rejects the request. If the API returns no matching job, the command prints `unknown` and exits successfully; a printed `failed` job status also does not make the command exit with an error.
...
- The command needs a project ID and either a stored login or an API key.
