...
2. Uploads the content, saves edits to previous local translations when [`options.saveLocal`](/docs/cli/reference/config#save-local) is enabled, and enqueues translation jobs. The `--save-local` and `--no-save-local` flags override that setting for one run. It does not download or publish results in the same run. Review-gated Projects hold completed translations for approval.
