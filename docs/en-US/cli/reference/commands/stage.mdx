...
Once `stageTranslations` is enabled, running [`translate`](/docs/cli/reference/commands/translate) without first staging that version causes the CLI to exit with an error. After translations are approved, run [`translate`](/docs/cli/reference/commands/translate) to download them.
