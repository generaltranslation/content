...
- Uses the API key from `--api-key` or `GT_API_KEY` when set; otherwise, it uses the stored [`gt login`](/docs/cli/reference/commands/login).
...
# Read Project information with a stored login or credentials from the environment
...
- An explicit API key takes precedence over the stored user login.
