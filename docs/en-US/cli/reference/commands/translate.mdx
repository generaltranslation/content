...
4. Stages the collected content with [`gt stage`](/docs/cli/reference/commands/stage): it uploads sources, detects and saves local edits to previous translations when [`options.saveLocal`](/docs/cli/reference/config#save-local) is enabled, then enqueues translation jobs. The `--save-local` and `--no-save-local` flags override that setting for one run. Because Lottie layout processing is asynchronous, a project with `.lottie` files exits before this step when `stageTranslations` is `false`, without uploading or enqueuing anything, and directs you to the split flow.
...
| `--save-local, --no-save-local` | Enable or disable saving local edits before enqueuing. Overrides [`options.saveLocal`](/docs/cli/reference/config#save-local). | `boolean` | Yes | `true` |
...
- **Local edits:** local edits are saved before translation by default. Set [`options.saveLocal`](/docs/cli/reference/config#save-local) to `false` to disable the step by default, or override the setting for one run with `--save-local` or `--no-save-local`.
