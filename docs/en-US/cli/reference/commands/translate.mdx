...
| `--save-local` | Save local edits to translation files before enqueuing. Overrides [`options.saveLocal`](/docs/cli/reference/config#save-local). | `boolean` | Yes | `true` |
| `--no-save-local` | Skip saving local edits before enqueuing. Overrides [`options.saveLocal`](/docs/cli/reference/config#save-local). | `boolean` | Yes | — |
...
- **Local edits:** before enqueuing, the CLI sends any hand edits to downloaded translation files back to the platform, the same step [`gt save-local`](/docs/cli/reference/commands/save-local) runs on its own. Disable it for one run with `--no-save-local`, or for every run with the [`options.saveLocal`](/docs/cli/reference/config#save-local) config key.
