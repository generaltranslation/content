...
*Note: Requires a project ID and [saved login or explicit tooling key](/docs/cli/guides/configuring#credentials), with file and translation queue permissions and context access when used. A generate-only runtime key is not sufficient.*
...
```bash
# Translate the configured project using saved login or an explicit tooling key
npx gt translate
...
- **Security:** never store your API key in `gt.config.json`. Explicit keys take precedence over saved login and never fall back on failure. (See [credential precedence](/docs/cli/guides/configuring#credentials)).
