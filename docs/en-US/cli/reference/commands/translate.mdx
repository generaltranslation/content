...
*Note: Set a project ID and authenticate with [`gt login`](/docs/cli/reference/commands/login) for interactive work, or with a project key in `GT_API_KEY` for CI. Grant file and translation queue permissions to an API key, and never store it in `gt.config.json`.*
...
```bash
# Translate using gt.config.json and a stored login
npx gt translate
...
- **Authentication:** an explicit API key takes precedence over the stored user login. Never store an API key in `gt.config.json`.

### Version history

| Version | Changes |
| --- | --- |
| `2.22.0` | Added stored user login as an alternative to a project API key for interactive workflows. |
