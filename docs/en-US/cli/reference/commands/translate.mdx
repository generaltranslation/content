...
2. For `gt-next`, `gt-react`, and `gt-react-native` projects, scans your `src` globs for inline content — [`<T>`](/docs/react/reference/components/t) components and [`useGT`](/docs/react/reference/hooks/use-gt) calls — and includes your dictionary file.
...
These apply when scanning source code in `gt-next`, `gt-react`, and `gt-react-native` projects.
...
- **Branching:** pass `--enable-branching` to track translations per git branch. See [Track translations by branch](/docs/cli/guides/branching).
