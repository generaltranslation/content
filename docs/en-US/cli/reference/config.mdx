...
| [`omitConfigIds`](#omit-config-ids) | Keep generated version and branch IDs out of this file. | `boolean` | Yes | `false` |
...
## `omitConfigIds` [#omit-config-ids]

**Type** `boolean` · **Optional** · **Default** `false`

When `true`, translation commands do not persist the generated `_versionId` and `_branchId` fields in `gt.config.json`. Successful staging removes existing values. This reduces config-file conflicts when branches produce different translation versions.

[`gt git setup`](/docs/cli/reference/commands/git-setup) can enable this setting and configure structured merge drivers for `gt-lock.json` and generated GTJSON files.
...
<Callout type="warn">
  With CDN publishing enabled, omitting config IDs means remote cache and CDN consumers may load the latest available translations instead of a pinned version. The CLI warns when both settings are enabled.
</Callout>
...
| Version | Changes |
| --- | --- |
| `2.14.53` | Added `omitConfigIds` and the [`gt git setup`](/docs/cli/reference/commands/git-setup) workflow. |
