...
By default, the CLI does not publish translated files to the GT CDN. You can enable publishing globally, per-file, or per-command.
...
To publish only GT's internal translation format (`.gt.json`), use the `publish` key under `files.gt`:
