...
The project used for API and translation workflows. Login does not select it: bind the project here or through the environment. A `--project-id` flag overrides the environment value. When the config includes `projectId`, disagreement with either the flag or the resolved environment ID fails validation. Environment resolution also recognizes supported framework prefixes, not only `GT_PROJECT_ID`. (See [authentication and environment loading](/docs/cli/guides/configuring#credentials)).
...
The API origin used by CLI requests, including [`gt api`](/docs/cli/reference/commands/api). Set this only when your workflow uses a custom General Translation API endpoint. [`gt login`](/docs/cli/reference/commands/login) binds the session to this API; a stored token for another API is rejected. `GT_API_URL` overrides the login resource, not this request setting. (See [login resource settings](/docs/cli/reference/commands/login#storage)).
