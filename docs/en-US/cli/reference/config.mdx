...
When `autoderive` is enabled, the CLI compiler automatically treats interpolated values in `t()`, `gt()`, and `msg()` calls as [`derive()`](/docs/next/api/strings/derive) calls. Instead of creating a single translation with a variable slot, it generates separate translation entries for each possible value — preserving word agreement, conjugation, and word order across languages.

Autoderive is **disabled by default**. To enable it, set `autoderive` to `true` in `files.gt.parsingFlags`:
...
// With autoderive enabled:
gt(`Welcome back, ${role}!`);
// Equivalent to: gt(`Welcome back, ${derive(role)}!`)
// Generates two translation entries:
//   "Welcome back, Admin!" → "¡Bienvenido de nuevo, Admin!"
//   "Welcome back, User!"  → "¡Bienvenido de nuevo, User!"

// With autoderive disabled (default):
gt(`Welcome back, ${role}!`);
// ❌ Error — interpolated values require an explicit derive() call
```

Without autoderive, passing interpolated values directly to `t()`, `gt()`, or `msg()` is treated as an error by the compiler. You must either enable autoderive or explicitly wrap values with [`derive()`](/docs/next/api/strings/derive).

<Callout>
The `t` tagged template macro is unaffected by autoderive — it handles derivation through its template literal syntax.
</Callout>

See also: [Compiler autoderive](/docs/next/concepts/compiler#autoderive) | [`derive()` API reference](/docs/next/api/strings/derive)
