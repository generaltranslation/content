...
- `output` — for `gt` files only, the local save path with a `[locale]` placeholder, such as `public/i18n/[locale].json`. Required when Locadex's **Preserve local edits** feature is enabled and `publish` is not set.
...
### Locadex integration requirements

When using Locadex automations with the **Preserve local edits** feature enabled:

- If `publish` is not set to `true`, `files.gt.output` must be configured. Locadex needs a local path to save translated GT files when they're not being published to the CDN.
- This ensures translated files are saved to your repository where Locadex can preserve manual edits across translation runs.
