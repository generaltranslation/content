...
The `[locale]`, `{locale}`, and `{localeCode}` placeholders preserve each locale tag exactly as written in `defaultLocale` or `locales`. For example, a configured `fr-ca` locale stays `fr-ca` in file paths and JSON locale keys. Property placeholders such as `{languageCode}`, `{regionCode}`, `{minimizedCode}`, and `{maximizedCode}` use their derived canonical values.
