...
*Note: Add the [JSON Schema](https://assets.gtx.dev/config-schema.json) with a `$schema` key for editor validation and autocompletion. The published schema lags some valid file keys, including `pot`, `html`, `txt`, `twilioContentJson`, `lottie`, `dotStrings`, `dotStringsdict`, `androidStrings`, and `xcstrings`. It also omits `fonts` and `options.saveLocal`, and accepts only a boolean for `options.experimentalLocalizeStaticUrls` instead of the valid `skipUntranslatedPages` object. Editors may flag unsupported schema fields until the schema is refreshed.*
...
| [`options.experimentalLocalizeStaticUrls`](#localize-static-urls) | Localize static links in Markdown and MDX output. | `boolean \| object` | Yes | `false` |
| [`options.saveLocal`](#save-local) | Detect and submit local translation edits before enqueueing. | `boolean` | Yes | `false` |
...
The target locales to translate into. See [supported locales](/docs/platform/dashboard/reference/supported-locales) for accepted codes. Framework initializers that accept a locale list also use these as the locales your app supports.
...
`dotStrings` and `dotStringsdict` require `gt` 2.18.1 or later, `androidStrings` requires `gt` 2.19.0 or later, and `xcstrings` requires `gt` 2.21.0 or later.
...
## `options.experimentalLocalizeStaticUrls` [#localize-static-urls]

**Type** `boolean | { skipUntranslatedPages?: boolean }` · **Optional** · **Default** `false`

Localizes root-relative links in translated Markdown and MDX files. Set it under the top-level `options` object:

- `true` adds the target locale to every matching static link.
- `{ "skipUntranslatedPages": true }` keeps a link on the source locale when its source page exists but no translated page exists. Links to generated pages without a source Markdown or MDX file, such as OpenAPI reference pages, are still localized.

Use `skipUntranslatedPages` for sites such as Mintlify, where a missing localized page returns a 404. Keep the boolean form for sites that serve untranslated content from localized URLs.
...
| Version | Changes |
| --- | --- |
| `2.21.5` | Added the `skipUntranslatedPages` object form. |

## `options.saveLocal` [#save-local]

**Type** `boolean` · **Optional** · **Default** `false`

Detects edits in previously downloaded local translation files and submits their diffs before [`gt translate`](/docs/cli/reference/commands/translate) or [`gt stage`](/docs/cli/reference/commands/stage) enqueues new work. Set it under the top-level `options` object. The `--save-local` and `--no-save-local` flags override this setting for one run.
