...
| [`options.saveLocal`](#save-local) | Save local edits to translation files before enqueuing. | `boolean` | Yes | `true` |
...
When `true`, translated files are published to the General Translation CDN after [`translate`](/docs/cli/reference/commands/translate), `upload`, or `save-local`. See [CDN publishing](#cdn-publishing) for per-file and per-command control.
...
When `true`, translations require human review before they are served. The CLI sets this automatically the first time you run [`gt stage`](/docs/cli/reference/commands/stage). Once enabled, `translate` only downloads translations for versions you have staged with `gt stage`; if nothing has been staged, there is nothing to download.
...
When `true`, [`translate`](/docs/cli/reference/commands/translate) and [`stage`](/docs/cli/reference/commands/stage) detect edits you have made to downloaded translation files and send them to the platform as diffs before enqueuing new translations, so a translation run never overwrites hand edits. This is the same step [`gt save-local`](/docs/cli/reference/commands/save-local) runs on its own. Set it to `false` to skip the step on every run, for example in CI where translation files are never edited by hand.

The `--save-local` and `--no-save-local` flags override this key for a single run.
