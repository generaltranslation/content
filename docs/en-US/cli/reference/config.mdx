...
- `output` — for `gt` files only, the local save path with a `[locale]` placeholder, such as `public/i18n/[locale].json`. Required when using Locadex's **Preserve local edits** feature without CDN publishing (see [Locadex integration requirements](#locadex-requirements)).
...
## Locadex integration requirements [#locadex-requirements]

When you use Locadex's **Preserve local edits** feature to save translation edits back to your local files, and you have not enabled CDN publishing with [`publish`](#publish), you must configure `files.gt.output` so the CLI knows where to save translated GT files locally.
...
The CLI validates this requirement when you run commands that interact with Locadex-managed translations. Without `files.gt.output` configured, the CLI cannot determine where to save the locally edited translations, and the command will fail.

When [`publish`](#publish) is enabled, the CLI publishes translations to the CDN instead, so the local `output` path is optional (though still recommended for local development).
