...
*Note: Add the [JSON Schema](https://assets.gtx.dev/config-schema.json) with a `$schema` key for editor validation and autocompletion. The published schema does not yet include the `lottie` and `fonts` keys added in `gt` 2.16.0, so editors may flag those valid settings until the schema is refreshed.*
...
The target locales to translate into. See [supported locales](/docs/platform/dashboard/reference/supported-locales) for accepted codes. When you use `gt-next`, `gt-react`, or `gt-vue`, these are also the locales your app supports.
...
| --- | --- | --- |
| `gt` | General Translation files for `gt-next`, `gt-react`, `gt-react-native`, and `gt-vue`. | [GT](/docs/cli/reference/formats/gt-jsx-files) |
...
- `parsingFlags` — for `gt` files only, flags that control inline content parsing. See [`autoderive`](/docs/cli/guides/using-autoderive), [automatic JSX injection](/docs/cli/guides/using-auto-jsx), and [Vue parsing flags](#vue-parsing).
...
**Type** `string[]` · **Optional** · **Default** Framework source globs

An array of glob patterns for source files scanned for inline content. React frameworks scan JavaScript and TypeScript files under `src`, `app`, `pages`, and `components`. `gt-vue` also scans root `.vue` files and Vue, Nuxt, and shared source directories including `composables`, `layouts`, `plugins`, `server`, `stores`, and `views`.
...
## Vue parsing flags [#vue-parsing]

`gt-vue` projects may set these keys under `files.gt.parsingFlags` when automatic Vue configuration discovery is not sufficient.

| Property | Description | Type | Optional | Default |
| --- | --- | --- | --- | --- |
| `viteConfigPath` | Project-relative path to the active Vite config. | `string` | Yes | Discovered Vite or Nuxt config |
| `vueCompilerOptions` | Hash-affecting Vue template compiler options. | `object` | Yes | Discovered compiler options |

`viteConfigPath` must point to a readable JavaScript or TypeScript config inside the project root and outside `node_modules`. Set it when both Nuxt and Vite config files exist or when the active Vite config has a nonstandard path.

`vueCompilerOptions` accepts only:

- `whitespace` — `condense` or `preserve`.
- `delimiters` — a two-string tuple such as `["${", "}"]`.

The CLI combines explicit values with statically analyzable Vite or Nuxt configuration. Conflicting or dynamic hash-affecting options stop extraction instead of producing catalog hashes that differ from Vue's runtime behavior.
