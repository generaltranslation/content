...
title: Apple .stringsdict
description: Translate Apple .stringsdict files with the General Translation CLI. API reference for the .stringsdict file format.

---

The CLI translates Apple `.stringsdict` files, the XML property lists that carry an app's plural rules. Each locale has its own file alongside its `.strings` tables in a `<locale>.lproj` directory. Translations keep the plist structure, the format keys, and the format specifiers, and rebuild the plural forms for each target locale.
