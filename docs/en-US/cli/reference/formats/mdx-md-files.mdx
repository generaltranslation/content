...
- `--experimental-localize-static-urls` — localizes URLs in translated files. Use [`options.experimentalLocalizeStaticUrls`](/docs/cli/reference/config#localize-static-urls) in `gt.config.json` when links to pages without translations should stay on the source locale.
