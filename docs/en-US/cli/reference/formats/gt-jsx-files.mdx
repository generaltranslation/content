...
- `autoderive` — parses interpolated values in `t()`, `gt()`, and `msg()` calls and expressions inside `<T>` components as if wrapped in `derive()`, generating a separate translation entry per possible value. Accepts `true` or an object such as `{ "jsx": true, "strings": false }`. See [Auto-derive translations](/docs/cli/guides/using-autoderive).
