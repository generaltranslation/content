...
title: SRT
description: Translate SubRip subtitle files while preserving timing and structure. Reference for SRT files.
...
| [Validation](#validation) | Skip empty files in the CLI and reject non-empty documents without a valid cue in the API. |
...
- Cue numbers
- Timing lines, including trailing position data
- Blank-line separators and content outside valid cues
- Subtitle markup such as `<i>`, `<b>`, `<u>`, `<font>`, and leading `{\an1}`–`{\an9}` position tags

Translated lines use the cue's detected source line ending, while untouched separators remain unchanged. Existing translations for unchanged cues can be reused. The [`--force`](/docs/cli/reference/commands/translate#flags) flag bypasses all reuse, while a Glossary retranslation refreshes cues around changed terms.
...
If the source file was moved or deleted after upload, the CLI uses the existing translated file's encoding. It defaults to UTF-8 when neither file is available.
