...
*Note: `.strings` and `.stringsdict` are the legacy Apple localization layout. Xcode's String Catalog migration folds the `.lproj` files into a single [`.xcstrings` catalog](/docs/cli/reference/formats/xcstrings-files) and deletes them, so a project uses one layout or the other.*
