...
As you can see, the `<Var>` component should be used to wrap any content that should remain the same regardless of locale.
...
<Callout>
    **Exceptions**
    ...
    However, this is not the intended use of the `<Var>` component or the General Translation API and doing so can harm load times and performance.
