...
**Do I have to use the platform to use the libraries?** No. The General Translation i18n libraries are free, open-source, MIT-licensed, and can be configured to load translations from any source. See the [React quickstart](/docs/react/react-quickstart) or [Vue quickstart](/docs/vue/quickstart).

**Which library should I use?** Use `gt-react` for React apps, `gt-next` for Next.js, and `gt-vue` for Vue. Use the [Node](/docs/node/quickstart) or [Python](/docs/python/quickstart) libraries for backend services, and the [Core library](/docs/platform/core/quickstart) for lower-level control in any JavaScript runtime.
