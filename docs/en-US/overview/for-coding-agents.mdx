...
```bash
GT_API_KEY="gtx-api-..."   # the same project key works in development, CI, and production
GT_PROJECT_ID="..."
```
...
Connect through your MCP client's OAuth sign-in flow, or configure `Authorization: Bearer <api-key>` using its secret-header settings. API-key connections accept project keys (`gtx-api-`) and Organization keys (`gtx-org-`). Each tool requires the matching permission on the key.
