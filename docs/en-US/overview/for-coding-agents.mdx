...
In Vue single-file components, import `<T>` and `useGT()` from `gt-vue`. Use `<T>` in the template, and call the function returned by `useGT()` from a template or computed value when the result must update reactively.
