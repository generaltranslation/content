...
The General Translation API also exposes a streamable HTTP MCP endpoint for project data at `https://api.gtx.dev/mcp`. It provides:

- `list_projects` to list the projects the authenticated identity can read. It returns up to 50 projects by default; pass `nextCursor` back as `cursor` to load another page, or set `limit` from 1 to 100.
- `get_project_info` to read a selected project's locale and approval settings.
- `get_translation_status` to read source metadata and per-locale translation status for a file in the selected project.

For an interactive MCP client, configure only the server URL and let the client use OAuth 2.1:

```json title=".mcp.json"
{
  "mcpServers": {
    "generaltranslation-projects": {
      "type": "streamable-http",
...
The server advertises its protected-resource and authorization-server metadata. An OAuth-capable client registers dynamically, uses the Authorization Code flow with Proof Key for Code Exchange (PKCE), and opens the Dashboard consent screen. Approve the `project:files:read` scope to use these tools. Request `offline_access` when the client needs a refresh token.

For non-interactive automation, authenticate through the MCP client's secret-header configuration with `Authorization: Bearer <api-key>`. Use a production project key (`gtx-api-`), or an Organization key (`gtx-org-`). Development keys are not accepted. The key needs `project:files:read`. Use `list_projects`, then pass a returned `projectId` to the project tools.
