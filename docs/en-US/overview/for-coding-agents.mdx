...
```bash
GT_API_KEY="gtx-api-..."   # Project keys use the same format in every environment
GT_PROJECT_ID="..."
```
...
Connect through your MCP client's OAuth sign-in flow, or configure `Authorization: Bearer <api-key>` using its secret-header settings. API-key connections accept project keys (`gtx-api-`) and Organization keys (`gtx-org-`). Limit the key to the permissions required by the tools you use.

The server advertises its protected-resource and authorization-server metadata. An OAuth-capable client registers dynamically, uses the Authorization Code flow with Proof Key for Code Exchange (PKCE), and opens the Dashboard consent screen. Request `openid` and `profile` for identity claims, and request `offline_access` when the client needs a refresh token. Dynamically registered clients must request individual API permission scopes; the `gt:*` wildcard is reserved for registered first-party clients.
