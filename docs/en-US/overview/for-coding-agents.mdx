...
```bash
npm install gt-next   # or gt-react / gt-vue / gt-node / generaltranslation
npm install -D gt
```
...
For Vue, use `<T>` and `<Var>` in templates, and install one `createGT()` plugin with a catalog loader. Vue `<Var>` receives its value as slot content, not through `name` or `value` props. Follow the [Vue Quickstart](https://generaltranslation.com/docs/vue/quickstart) because the unstable 0.x API differs from the React packages.
