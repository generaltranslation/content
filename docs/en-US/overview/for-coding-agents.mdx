...
Configure your MCP client to send `Authorization: Bearer <api-key>`. Use a production Project key (`gtx-api-`), or use an Organization key (`gtx-org-`) and send the target Project ID in the `gt-project-id` header. Development keys are not accepted. The key needs `project:files:read`.
