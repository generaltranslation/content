...
The docs host also serves the root files under `/docs`, including `/docs/llms.txt`, `/docs/llms-index.txt`, and `/docs/llms-full.txt`. Every docs page is available as **raw Markdown**: append `.mdx` to any page URL (for example, `/docs/cli/quickstart.mdx`) to fetch the clean source instead of parsing rendered HTML. The `.md` suffix remains supported for compatibility.
