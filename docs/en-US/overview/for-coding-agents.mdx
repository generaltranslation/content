...
| `npx gt project create` / `npx gt project status` | To provision a Project with an Organization key or inspect a setup job. |
...
Authenticate through your MCP client's secret-header configuration with `Authorization: Bearer <project-api-key>`. The key needs `project:files:read`. When using an Organization API key, also send the target Project ID in the `gt-project-id` header.
