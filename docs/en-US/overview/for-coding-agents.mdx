...
description: How to use AI coding agents and LLMs with General Translation by pointing them at the machine-readable docs, MCP servers, and our drop-in agent guide.
...
| `npx gt api --spec` | To inspect the API contract bundled with the installed CLI before making a direct API request. |
...
General Translation provides [Model Context Protocol](https://modelcontextprotocol.io) (MCP) servers for documentation and authenticated Project data.

### Documentation server

The documentation server lets agents query the docs directly. It comes in two forms:
...
The documentation server provides `fetch-docs` for one page and `list-docs` for the short or full machine-readable docs index.
...
The Project server exposes read-only Project information at `https://api.gtx.dev/mcp` over streamable HTTP. It requires a production Project API key with `project:files:read`; development keys are rejected. An Organization key can also connect when the request includes the target `gt-project-id`.

Configure a Project-scoped connection in clients that support custom MCP headers:

```json title=".mcp.json"
{
  "mcpServers": {
    "generaltranslation-project": {
      "type": "streamable-http",
...
Store the key with your client's secret-management feature instead of committing it to `.mcp.json`. See [API keys](/docs/platform/dashboard/reference/api-keys) for key types and permissions.

For an Organization key, add `"gt-project-id": "<PROJECT_ID>"` to `headers`. The server allows 200 requests per minute for each API key.

The server provides:

- `get_project_info` — returns the Project ID, name, Organization ID, source locale, target locales, and auto-approval setting.
- `get_translation_status` — accepts a `fileId` plus optional `versionId` and `branchId`, then returns source metadata and timestamps for each target locale.

Tool-level failures still use an HTTP `200` MCP response. Check the tool result's `isError` field and JSON error content; failed results do not include `structuredContent`.
