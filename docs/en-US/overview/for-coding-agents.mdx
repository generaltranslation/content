...
CLI login, runtime development keys, and CI tooling keys are separate. SDKs do not read CLI sessions; init's generate-only key does not replace `GT_API_KEY`. Never commit keys or include them in deployed browser/mobile bundles.

Before configuring or troubleshooting authentication, read [CLI credentials](/docs/cli/guides/configuring#credentials). Keep human approval for login and deliberate approval for key creation. `--no-browser` and quiet mode are not unattended login; use manual config and scoped keys in CI.
...
| `npx gt configure` | Interactive configuration without the React rewrite; may install dependencies and provision runtime credentials. Write config manually for config-only changes. |
