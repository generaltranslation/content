...
Connect through your MCP client's OAuth sign-in flow, or configure `Authorization: Bearer <api-key>` using its secret-header settings. API-key connections accept production project keys (`gtx-api-`) and Organization keys (`gtx-org-`); development keys are not accepted.
