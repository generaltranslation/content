...
Vue support is currently available under the `iris` prerelease tag. For Vue, run `npx gt@iris init` instead.
...
For Vue, install the matching prerelease packages:

```bash
npm install gt-vue@iris
npm install -D gt@iris
```
