...
General Translation offers a hosted [Model Context Protocol](https://modelcontextprotocol.io) (MCP) server at `https://api.gtx.dev/mcp` for agents that need live project information. It uses streamable HTTP.

For documentation questions, use the [machine-readable docs](#point-agents).
...
Connect through your MCP client's OAuth sign-in flow, or configure `Authorization: Bearer <api-key>` using its secret-header settings. API-key connections accept production project keys (`gtx-api-`) and Organization keys (`gtx-org-`); development keys are not accepted. The key needs `project:files:read`.
