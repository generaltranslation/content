...
- **Direct TypeScript calls to the public API** → `@generaltranslation/api`
- **Translating content files (JSON, MDX, YAML, and more) or running translation in CI** → the `gt` CLI

All of these are free and open-source. The i18n libraries work with or without a General Translation account; direct API calls and hosted translation require an API key.

## Setup

For a direct API integration, install `@generaltranslation/api` and follow the [TypeScript SDK reference](/docs/platform/openapi/reference/typescript-sdk).
...
- [TypeScript SDK](/docs/platform/openapi/reference/typescript-sdk) — generated functions and types for direct API calls.
