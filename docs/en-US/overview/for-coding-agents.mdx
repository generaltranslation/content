...
For a new setup with a human present, run from the app directory:

```bash
npx gt init
```

The wizard starts sign-in if needed, detects the framework, creates or updates `gt.config.json`, and can select/create a project and provision a generate-only development runtime key. Framework rewrites are experimental and need review. Dependency installation and loaders depend on the selected workflow; local Vite does not necessarily install `gt` or create credentials. Review [init side effects](/docs/cli/reference/commands/init) before running it.
...
CLI login, runtime development keys, and CI tooling keys are separate. SDKs do not read CLI sessions; init's generate-only key does not replace `GT_API_KEY`. App Router uses private `GT_PROJECT_ID` / `GT_DEV_API_KEY`; Pages Router browser previews and Vite use their public prefixes only for local development. Never commit keys or include them in deployed browser/mobile bundles.

Before troubleshooting authentication, read [CLI credential precedence](/docs/cli/guides/configuring#credentials): explicit keys override login, invalid keys never fall back, and loaded env files can override exported keys. Keep human approval for login and deliberate approval for key creation. `--no-browser` and quiet mode are not unattended login; use manual config and scoped keys in CI.
...
| Command                          | When to run                                                                                                                                                         |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `npx gt init` | Guided setup with a human present; can change framework files, dependencies, config, and runtime credentials. |
| `npx gt configure` | Skip the experimental React rewrite, but still potentially generate loaders, install the CLI, log in, and provision runtime credentials. Write config manually for config-only changes. |
| `npx gt login` | Have the developer approve account access for CLI work. |
| `npx gt whoami` | Check the signed-in account over the network. |
| `npx gt logout` | Remove the saved session; does not revoke API keys. |
| `npx gt api-key create --project-id <id> --name <name> --permission project:translations:generate` | Only with explicit approval, create a generate-only runtime key. Stdout is the secret, not JSON; store it securely without echoing or logging it. Choose full-workflow grants separately for CI. |
...
- For account or key work, read [CLI credentials](/docs/cli/guides/configuring#credentials), [`gt login`](/docs/cli/reference/commands/login), and [`gt api-key create`](/docs/cli/reference/commands/api-key-create).
...
Connect through your MCP client's OAuth sign-in flow, or configure `Authorization: Bearer <api-key>` using its secret-header settings. API-key connections accept project keys (`gtx-api-`) and Organization keys (`gtx-org-`). Each tool checks the key's permissions for the requested action. Configure [custom key permissions](/docs/platform/dashboard/reference/api-keys) for the tools your agent needs.
...
- **Hand to the agent:** wrapping user-facing copy in [`<T>`](/docs/react/reference/components/t), adding [`useGT()`](/docs/react/reference/hooks/use-gt) for standalone strings, and scaffolding `gt.config.json`. Run [`npx gt init`](/docs/cli/reference/commands/init) only with a human present to approve setup and provisioning.
