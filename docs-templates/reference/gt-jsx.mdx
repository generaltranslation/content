...
Where `key` is the hash of the original language `GTJSXTree`,
...
The `id` prop does not change the key used to look up a translation.
Keys are always hashes computed from the message content plus its `context` and `maxChars` metadata (and review metadata).
...
Here, `example` is descriptive metadata sent to the GT platform; the translation is still stored and looked up under the content hash.
