...
Shared strings are text values used in multiple places throughout your application - like navigation labels, form messages, or configuration data. Instead of duplicating translation logic everywhere, use [`msg`](__DOCS_PATH__/api/strings/msg) to mark strings for translation and [`useMessages`](__DOCS_PATH__/api/strings/use-messages) to decode them.
...
This creates maintenance overhead and makes your code harder to read. The [`msg`](__DOCS_PATH__/api/strings/msg) function solves this by letting you mark strings for translation in-place, then decode them when needed.

## Quickstart

Use [`msg`](__DOCS_PATH__/api/strings/msg) to mark strings and [`useMessages`](__DOCS_PATH__/api/strings/use-messages) to decode them:
...
1. **Mark Phase**: [`msg`](__DOCS_PATH__/api/strings/msg) encodes strings with translation metadata
2. **Decode Phase**: [`useMessages`](__DOCS_PATH__/api/strings/use-messages) decodes and translates the strings
...
<Callout type="warn">
  Encoded strings from [`msg`](__DOCS_PATH__/api/strings/msg) cannot be used directly - they must be decoded with [`useMessages`](__DOCS_PATH__/api/strings/use-messages).
</Callout>

## Components

Use [`useMessages`](__DOCS_PATH__/api/strings/use-messages) hook:
...
Sometimes you need to access the original string without translation, such as for logging, debugging, or comparisons. Use [`decodeMsg`](__DOCS_PATH__/api/strings/msg#getting-original-strings-with-decodemsg) to extract the original text:
...
Never use the output of [`msg`](__DOCS_PATH__/api/strings/msg) directly:
...
Every [`msg`](__DOCS_PATH__/api/strings/msg) string needs to be decoded:
...
- [Dictionaries Guide](__DOCS_PATH__/guides/dictionaries) - Organize translations with structured data
- [Languages Guide](__DOCS_PATH__/guides/languages) - Configure supported languages
- API References:
  - [`msg` Function](__DOCS_PATH__/api/strings/msg)
