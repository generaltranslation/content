...
String translation provides direct access to text translations without JSX, perfect for attributes, object properties, and plain text values. Use [`useGT`](/docs/react/reference/hooks/use-gt) in __FRAMEWORK_NAME__ components for string translation.
...
Use the [`<T>` component](/docs/react/reference/components/t) for JSX content:
...
Follow React hook rules when using [`useGT`](/docs/react/reference/hooks/use-gt):
...
<Callout>
  For truly dynamic content that needs runtime translation, see the [Dynamic Content Guide](/docs/overview/key-concepts#content).
</Callout>

## Next steps

- [Dynamic Content Guide](/docs/overview/key-concepts#content) - Handle runtime translation
- [Shared Strings Guide](/docs/react/guides/translating-strings#shared) - Organize reusable translations
- API References:
