...
Branching components enable conditional content rendering within [`<T>`](__DOCS_PATH__/api/components/t) components. They handle dynamic logic like if/else statements and pluralization rules while ensuring all content variations can be properly translated.

## Available components

- [`<Branch>`](__DOCS_PATH__/api/components/branch): Conditional content based on values or states
- [`<Plural>`](__DOCS_PATH__/api/components/plural): Automatic pluralization using locale-specific rules

## Quickstart

Branching components work inside [`<T>`](__DOCS_PATH__/api/components/t) to handle conditional logic:
...
1. **Replacing ternary operators** and conditional logic inside [`<T>`](__DOCS_PATH__/api/components/t)
...
Use [`<Branch>`](__DOCS_PATH__/api/components/branch) for any conditional rendering based on values or states:
...
Use [`<Plural>`](__DOCS_PATH__/api/components/plural) for content that changes based on quantity:
...
Convert conditional logic for use within [`<T>`](__DOCS_PATH__/api/components/t):
...
Branching components can be used outside [`<T>`](__DOCS_PATH__/api/components/t) for pure logic without translation:
...
- API References:
  - [`<Branch>` Component](__DOCS_PATH__/api/components/branch)
