...
Then register it with `initializeGT()` at your app's entry point, before rendering:
...
Register it with `initializeGT()` alongside your default-locale dictionary:
...
<Callout type="info">
  **Note:** `loadDictionary` only runs for non-default locales. The default-locale dictionary must be bundled with your app and passed synchronously via the `dictionary` option.
</Callout>
