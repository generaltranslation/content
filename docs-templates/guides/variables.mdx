...
Variable components allow you to safely include dynamic content within [`<T>`](__DOCS_PATH__/api/components/t) components. They handle formatting and localization locally without sending data to the translation API, making them perfect for sensitive information like usernames, account numbers, and financial data.

## Available components

- [`<Var>`](__DOCS_PATH__/api/components/var): Raw dynamic content without formatting
- [`<Num>`](__DOCS_PATH__/api/components/num): Numbers with locale-specific formatting
- [`<Currency>`](__DOCS_PATH__/api/components/currency): Currency values with symbols and formatting
- [`<DateTime>`](__DOCS_PATH__/api/components/datetime): Dates and times with locale conventions

## Quickstart

Variable components work inside [`<T>`](__DOCS_PATH__/api/components/t) to handle dynamic content:
...
1. **Wrapping dynamic values** so they can be used inside [`<T>`](__DOCS_PATH__/api/components/t)
...
Use [`<Var>`](__DOCS_PATH__/api/components/var) for any dynamic content that doesn't need special formatting:
...
Use [`<Num>`](__DOCS_PATH__/api/components/num) for numbers that should follow locale formatting rules:
...
Use [`<Currency>`](__DOCS_PATH__/api/components/currency) for monetary amounts:
...
Use [`<DateTime>`](__DOCS_PATH__/api/components/datetime) for dates and times:
...
### Important exception
Be careful with nested [`<T>`](__DOCS_PATH__/api/components/t) components inside variable components:
...
Variable components can be used outside of [`<T>`](__DOCS_PATH__/api/components/t) for pure formatting:
...
For [`<Currency>`](__DOCS_PATH__/api/components/currency), make sure to pass the `currency` prop to specify the currency type. This ensures that the correct currency symbol and formatting are used when displaying the value:
...
- [String Translation Guide](__DOCS_PATH__/guides/strings) - Translate plain text without JSX
- API References:
  - [`<Var>` Component](__DOCS_PATH__/api/components/var)
  - [`<Num>` Component](__DOCS_PATH__/api/components/num)
  - [`<Currency>` Component](__DOCS_PATH__/api/components/currency)
