---
title: Migrating
description: Learn how to migrate a project to __PACKAGE_NAME__
---

## Overview

This guide will cover the steps needed to migrate a project that's already using an i18n library to `__PACKAGE_NAME__`.
...
- A basic understanding of the `__PACKAGE_NAME__` library.

## Why migrate?

There are many reasons why you might want to migrate your project to `__PACKAGE_NAME__`.
...
<Steps>
  <Step>
    Install `__PACKAGE_NAME__` and the `gt` CLI tool.
...
<Tabs items={['npm', 'yarn', 'bun', 'pnpm']}>
  ...
  <Step>
    ...
    Call `initializeGT()` with your configuration at your app's entry point, then add the `<GTProvider>` component to the root of your app.
    ...
    ```
    For more detailed steps, see the [project quickstart](__DOCS_PATH__) guide.
  </Step>
  <Step>
    At this point, you have 3 options:

    1. Fully migrate your entire project to `__PACKAGE_NAME__`, and remove the old i18n library.
    2. Fully migrate your project, but keep using dictionaries from the old i18n library.
    2. Keep using the old i18n library for now, and only migrate part of your project to `__PACKAGE_NAME__`.
...
After you've set up your project, you'll need to search for all instances of your old i18n library and replace them with `__PACKAGE_NAME__`.
...
Let's say that you want to migrate your project to `__PACKAGE_NAME__`, but you want to keep using dictionaries from the old i18n library
...
Copy the contents of your old dictionary file into this new file, then register it with `initializeGT()` at your app's entry point.
...
See the [dictionaries](__DOCS_PATH__/guides/dictionaries) guide for more details.

### Option 3: Keep using the old i18n library for now, and only migrate part of your project to `__PACKAGE_NAME__`

This option is the most flexible, and will require the least code changes in one go.

In this case, you can do something similar to Option 2, but only migrate part of your project to `__PACKAGE_NAME__`.

For example, you can keep using the old i18n library for some components, and only use `__PACKAGE_NAME__` for others and for new content.
