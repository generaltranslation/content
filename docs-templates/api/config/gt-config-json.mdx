...
This file is read by (1) your [`<GTProvider>`](__DOCS_PATH__/api/components/gtprovider) component and (2) the [`gt translate`](/docs/cli/translate) command.
Because of this, we recommend storing your configuration in your `gt.config.json` file instead of passing it as a prop to your [`<GTProvider>`](__DOCS_PATH__/api/components/gtprovider) component.
...
|------|------|---------|-------------|
| `enableAutoJsxInjection` | `boolean` | `false` | Automatically wrap translatable JSX text in translation components at build time. See [auto JSX injection](__DOCS_PATH__/concepts/compiler#auto-jsx-injection). |
| `autoderive` | `boolean` | `true` | Automatically treat interpolated values in [`t()`](__DOCS_PATH__/api/strings/use-gt) calls as [`derive()`](__DOCS_PATH__/api/strings/derive) calls. See [autoderive](__DOCS_PATH__/concepts/compiler#autoderive). |
 ...
 * It is read by both the [`<GTProvider>`](__DOCS_PATH__/api/components/gtprovider) component and the [`gt translate`](/docs/cli/translate) command.
