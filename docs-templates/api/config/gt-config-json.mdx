...
This file is read by (1) your [`<GTProvider>`](/docs/react/reference/components/gt-provider) component and (2) the [`gt translate`](/docs/cli/reference/commands/translate) command.
Because of this, we recommend storing your configuration in your `gt.config.json` file instead of passing it as a prop to your [`<GTProvider>`](/docs/react/reference/components/gt-provider) component.
...
This means that they must be manually approved before they can be deployed to production via the [`gt translate`](/docs/cli/reference/commands/translate) command.
...
|------|------|---------|-------------|
| `enableAutoJsxInjection` | `boolean` | `false` | Automatically wrap translatable JSX text in translation components at build time. See [auto JSX injection](/docs/cli/guides/using-auto-jsx). |
| `autoderive` | `boolean` | `false` | Automatically treat interpolated values in `t()`, `gt()`, and `msg()` calls as [`derive()`](/docs/react/reference/functions/derive) calls. See [autoderive](/docs/cli/guides/using-autoderive). |
 ...
 * It is read by both the [`<GTProvider>`](/docs/react/reference/components/gt-provider) component and the [`gt translate`](/docs/cli/reference/commands/translate) command.
