...
|------|------|---------|-------------|
| `enableAutoJsxInjection` | `boolean` | `false` | Automatically wrap translatable JSX text in translation components at build time. See [auto JSX injection](/docs/cli/guides/auto-jsx). |
| `autoderive` | `boolean` | `false` | Automatically treat interpolated values in `t()`, `gt()`, and `msg()` calls as [`derive()`](/docs/react/reference/functions/derive) calls. See [autoderive](/docs/cli/guides/autoderive). |
