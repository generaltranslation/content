...
| `autoderive` | `boolean` | `false` | Automatically treat interpolated values in `t()`, `gt()`, and `msg()` calls as [`derive()`](__DOCS_PATH__/api/strings/derive) calls. See [autoderive](/docs/cli/reference/config#autoderive). |
