...
|-----------|-------------|
| `children` | The content to render inside the component. Typically a date or time value. |
...
import { DateTime } from '__PACKAGE_NAME__';
...
```jsx title="EventDate.jsx" copy
import { T, DateTime } from '__PACKAGE_NAME__';
...
## Next steps
 * For more details and usage examples of the `<DateTime>` component and other variable components like `<Currency>`, `<Num>`, and `<Var>`, see the [Using Variable Components](/docs/react/guides/formatting-variables) documentation.
