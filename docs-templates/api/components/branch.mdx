...
When the `branch` prop is missing or has no matching key, the component renders `children` (the fallback) instead of throwing.
...
import { Branch } from '__PACKAGE_NAME__';
...
```jsx title="BranchExample.jsx" copy
import { T, Branch } from '__PACKAGE_NAME__';
...
```jsx title="BranchExample.jsx" copy
import { Branch, T, Var } from '__PACKAGE_NAME__';
 ...
 * Combine `<Branch>` with other components, such as `<T>` for translations and [variable components](/docs/react/guides/formatting-variables) for dynamic content, to create rich and localized interfaces.

## Next steps
 * For more advanced usage and examples, refer to [Using Branching Components](/docs/react/guides/handling-plurals-and-branches).
