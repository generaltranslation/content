 ...
 * Combine `<Branch>` with other components, such as `<T>` for translations and [variable components](__DOCS_PATH__/guides/variables) for dynamic content, to create rich and localized interfaces.

## Next steps
 * For more advanced usage and examples, refer to [Using Branching Components](__DOCS_PATH__/guides/branches).
