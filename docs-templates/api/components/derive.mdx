...
import { T, Derive } from '__PACKAGE_NAME__';
...
import { T, Derive, Var } from '__PACKAGE_NAME__';
...
You can use `derive()` in the `context` prop of `<T>` to create context-specific translation variants.
This generates separate translation entries for each context value, allowing the same content to be translated differently depending on context.
...
This creates two translation entries:
- `"Hello, how are you?"` with context `"formal"` → "Bonjour, comment allez-vous ?"
- `"Hello, how are you?"` with context `"casual"` → "Salut, ça va ?"

### Combining with content derivation

You can use `<Derive>` for content and `derive()` for context in the same translation:
...
export default function Example({ gender }) {
  return (
    <T context={derive(getFormality())}>
      The <Derive>{getSubject(gender)}</Derive> is here.
    </T>
  );
}
```

This creates four translation entries (2 content variants × 2 context variants).
...
- For the string equivalent of `<Derive>`, see [`derive`](__DOCS_PATH__/api/strings/derive).