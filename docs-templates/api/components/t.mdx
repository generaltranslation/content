...
Make sure to follow the [deployment guide here](/docs/react/react-quickstart).
</Callout>
...
| `maxChars` | The maximum number of characters allowed in the translated output. `$maxChars` is an equivalent alias. |
...
Remember to add a project API key with runtime translation permission to your environment to enable this behavior.
...
Remember, the `<T>` component must be used inside a [`<GTProvider>`](/docs/react/reference/components/gt-provider) to access the translations.
 ...
 * Ensure the `<T>` component is wrapped in a [`<GTProvider>`](/docs/react/reference/components/gt-provider) to access the translation context.
...
## Next steps
 * For more advanced features like on-demand translation, variables, context, and handling plurals, refer to the [`<T>` design patterns](/docs/react/guides/translating-jsx) documentation.
 * For translating strings, look into [`useGT`](/docs/react/reference/hooks/use-gt).
