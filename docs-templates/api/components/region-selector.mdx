...
It is a client-side component that reads region data from the [`<GTProvider>` context](/docs/react/reference/components/gt-provider).
...
- For a fully custom region selector UI, use the [`useRegionSelector`](/docs/react/reference/hooks/use-region-selector) hook.

## Next steps

- See [`useRegionSelector`](/docs/react/reference/hooks/use-region-selector) for building a custom region selector.
- See [`useRegion`](/docs/react/reference/hooks/use-region) to read the current region.
- See [`<LocaleSelector>`](/docs/react/reference/components/locale-selector) for the locale equivalent.
