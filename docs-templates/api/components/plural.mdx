...
## Next steps
 * For more examples, check out the reference doc on [Using Branching Components](__DOCS_PATH__/guides/branches).
 * For more advanced usage, combine `<Plural>` with variable components like `<Currency>`, `<DateTime>`, `<Num>`, and `<Var>`. Read more about [Using Variable Components](__DOCS_PATH__/guides/variables).
