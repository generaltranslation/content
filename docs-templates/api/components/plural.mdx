...
## Next steps
 * For more examples, check out the reference doc on [Using Branching Components](/docs/react/guides/plurals-and-branches).
 * For more advanced usage, combine `<Plural>` with variable components like `<Currency>`, `<DateTime>`, `<Num>`, and `<Var>`. Read more about [Using Variable Components](/docs/react/guides/format-variables).
