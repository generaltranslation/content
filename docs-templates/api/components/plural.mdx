...
import { Plural } from '__PACKAGE_NAME__';
...
```jsx title="PluralExample.jsx" copy
import { T, Plural, Num } from '__PACKAGE_NAME__';
...
## Next steps
 * For more examples, check out the reference doc on [Using Branching Components](/docs/react/guides/handling-plurals-and-branches).
 * For more advanced usage, combine `<Plural>` with variable components like `<Currency>`, `<DateTime>`, `<Num>`, and `<Var>`. Read more about [Using Variable Components](/docs/react/guides/formatting-variables).
