...
It is used with [`useGT`](/docs/react/reference/hooks/use-gt) and [`msg`](/docs/react/reference/functions/msg) to pass variables to inline string translations.
...
| `$requiresReview` | Optionally include `$requiresReview` as a variable to gate the translation behind a human-review step before it is served. Must be a boolean literal. |
...
## Next steps
 * See [`useGT`](/docs/react/reference/hooks/use-gt) for more information on inline string translations.
