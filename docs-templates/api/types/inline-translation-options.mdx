...
| `$context` | Optionally include `$context` as a variable in the `variables` object to provide context for the content (used for translation). Accepts a string or `derive()` for [context derivation](__DOCS_PATH__/api/strings/derive#context-derivation). |
