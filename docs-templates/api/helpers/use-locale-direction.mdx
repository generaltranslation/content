...
The `useLocaleDirection` hook retrieves the text direction (`'ltr'` or `'rtl'`) for the current or a specified locale from the [`<GTProvider>` context](/docs/react/reference/components/gt-provider).
