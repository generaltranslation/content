...
```jsx copy
import { derive, gt } from '__PACKAGE_NAME__';
...
You can use `derive` inside the `$context` parameter to create context-specific translation variants.
This generates separate translation entries for each context value, allowing the same source text to be translated differently depending on context.
...
function Component() {
  const translation = gt("Hello, how are you?", {
...
This creates two translation entries, one for each context:
...
You can use `derive` in both the content and the `$context` parameter:
