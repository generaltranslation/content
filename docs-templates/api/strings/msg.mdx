...
The encoded string should be passed to the [`useMessages`](/docs/react/reference/hooks/use-messages) hook to retrieve translations.
...
```jsx
import { msg, decodeMsg } from '__PACKAGE_NAME__';
const encodedString = msg('Hello, world!');
...
| `options?`            | [`InlineTranslationOptions`](/docs/react/reference/types/inline-translation-options) | Translation options to customize the behavior of `msg`. |
...
## Next steps
 * See [`useMessages`](/docs/react/reference/hooks/use-messages) for translating strings.
