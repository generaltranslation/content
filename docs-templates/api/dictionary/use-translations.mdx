...
`useTranslations` is used to access string translations from the [translation dictionary](/docs/react/guides/translating-with-dictionaries).

It must be used within a component wrapped by a [`<GTProvider>`](/docs/react/reference/components/gt-provider).
...
<Callout>
  `useTranslations` uses a [dictionary](/docs/react/guides/translating-with-dictionaries) to store all content for translation.
  This is different from using the [`<T>` component](/docs/react/guides/translating-jsx) for translation.
...
| `options?`            | [`DictionaryTranslationOptions`](/docs/react/reference/types/dictionary-translation-options) | Translation options to customize the behavior of `d`. |
 ...
 * The `useTranslations` hook can only be used within a component wrapped by a [`<GTProvider>` component](/docs/react/reference/components/gt-provider).
