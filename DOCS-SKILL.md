...
- **Partner credit programs are the exception, and live in one place.** The partner credits section of the Dashboard **Managing billing** guide (`/docs/platform/dashboard/guides/managing-billing#partner-credits`) is the canonical list of accelerator and partner offers: one table row per program with its credit amount and validity period, each linked to its offer page on generaltranslation.com, with any partner perks portal linked in the prose below the table. Verify every amount and expiry against the live offer page before adding or changing a row, update the table when a program launches, changes, or ends, and link to the section from elsewhere instead of repeating amounts.
...
- **Links to generaltranslation.com pages outside the docs (pricing, enterprise, partner offers) are locale-less and canonical.** Write `https://generaltranslation.com/enterprise/contact/mercury`, never `/en-US/...` or `/en-GB/...`; the site redirects to a localized URL on its own. Use one canonical path per destination — do not link or mention vanity shortlinks or alternate paths for the same page — and reuse that exact form everywhere the page is referenced, including PR descriptions.
