...
gt-next provides the [`useLocale`](/docs/react/reference/hooks/use-locale) hook that makes this trivial in your root layout:
...
[`useLocale`](/docs/react/reference/hooks/use-locale) returns the BCP 47 locale code (e.g., `en-US`, `ar`, `zh-Hans`).
...
Combine it with gt-next's [`getLocale`](/docs/react/nextjs/reference/functions/get-locale) to generate the correct canonical for each locale:
...
Use gt-next's [`getGT`](/docs/react/nextjs/reference/functions/get-gt) function to translate metadata strings:
...
| ----------------------- | ------------------------------------------------------------------- |
| Locale in URL           | [`createNextMiddleware()`](/docs/react/nextjs/reference/functions/create-next-middleware) with `[locale]` dynamic segment            |
| HTML `lang` attribute   | [`useLocale()`](/docs/react/reference/hooks/use-locale) in root layout                                        |
| Canonical URLs          | [`getLocale()`](/docs/react/nextjs/reference/functions/get-locale) + Next.js metadata API `alternates.canonical`         |
| Hreflang tags           | Next.js metadata API `alternates.languages` with all supported locales |
| Translated metadata     | [`getGT()`](/docs/react/nextjs/reference/functions/get-gt) for page titles and descriptions                         |
