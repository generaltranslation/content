...
There are six things you need to get right, and this guide covers all of them using [gt-next](https://generaltranslation.com/docs/next/tutorials/quickstart).
...
gt-next provides `getLocale` on the server, which makes this trivial in your root layout:
...
`getLocale` returns the BCP 47 locale code (e.g., `en-US`, `ar`, `zh-Hans`).
...
| HTML `lang` attribute   | `getLocale()` in root layout                                        |
