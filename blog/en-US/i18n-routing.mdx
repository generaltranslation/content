...
Without locale-based routing, you end up storing the language in a cookie or query parameter:

```
myapp.com/about?lang=fr
```

This causes real problems. Search engines can't index your French and English pages separately. Users can't share a link to the French version — the recipient just sees whatever language is in their own cookie. And first-time visitors have no way to land directly on the right language.

The fix is straightforward: put the locale in the URL path and use Next.js middleware to handle detection and redirects.
