...
Our [`<Plural>`](/docs/react/reference/components/plural) component checks the exact plural category first, then the generic `plural` and `other` props. It falls back to its children only when none of those props are available.
...
gt-react also offers a [`<Num>`](/docs/react/reference/components/num) component which relies on `Intl.NumberFormat` [under the hood](https://github.com/General-Translation/gt-libraries/blob/main/packages/core/src/formatting/format.ts#L14C1-L27C2).
...
gt-react's [`<Plural>`](/docs/react/reference/components/plural) component:

- Is a simple, declarative way to render plurals correctly in React
- Works natively with the [`<Num>`](/docs/react/reference/components/num) formatting component
- Works natively with the [`<T>`](/docs/react/reference/components/t) translation component, which integrates with our [free translation service](https://generaltranslation.com) to generate plural forms automatically
