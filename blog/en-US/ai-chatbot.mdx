...
Oftentimes, developers are so focused on building the product that they forget about the users.
...
Existing libraries such as `next-intl` or `next-i18next` are a pain to set up and use. Not only that, but they don't even do any translations.
...
Next, I installed the [`gt-next`](https://github.com/General-Translation/gt) and [`gt`](https://github.com/General-Translation/gt) packages.
...
The crux of the library is the [`<T>`](/docs/react/reference/components/t) component.
...
Anything that is wrapped in the [`<T>`](/docs/react/reference/components/t) component can be translated.
The CLI setup tool uses a custom babel parser to scan through our codebase and
automatically wraps any React components it finds with a [`<T>`](/docs/react/reference/components/t).

Using the [`<T>`](/docs/react/reference/components/t) component has several advantages over other libraries:
...
I cleaned these up by importing the [`useGT`](/docs/react/reference/hooks/use-gt) hook from `gt-next` and using it to translate the strings:
