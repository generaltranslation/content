...
Your UI says one thing, your translation files say another, and a key-based mapping holds the two together. Here's what that looks like in practice:

```tsx
// What does this component render? You can't tell without opening en.json.
...
Reading this tells you almost nothing about what the user sees. You'd have to open `checkout.json`, find the `summary` namespace, and cross-reference five keys to understand the UI. Multiply that across hundreds of components and code review becomes an exercise in file-hopping.
...
The `<T>` component in [`gt-react`](https://generaltranslation.com/docs/react) marks a block of JSX for translation. The English stays in your component. When a user visits in Spanish, the content inside `<T>` is swapped for its Spanish equivalent, preserving the HTML structure, the formatting, everything.
...
In practice, though, most developers are already thinking in one language when they build UI. The layout, the copy, the button labels are all conceived in English first. That bias is already in the design whether the strings are inline or in a JSON file. Given that, we think the i18n framework should adapt to how you actually work, take the app you built naturally and handle the translation, rather than asking you to abstract away your content into keys for the sake of language neutrality.
